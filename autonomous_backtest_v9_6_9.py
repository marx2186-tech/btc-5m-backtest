...
RESULTS = ROOT / "results_v9_6_9"
REQUEST_PATH = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_6_9.json")))
...
ENGINE_NAME = "BTC 5m unified rating source and dual risk-budget validation V9.6.9"

spec = importlib.util.spec_from_file_location("v969_strategy_core", ROOT / "_v969_strategy_core.py")
if spec is None or spec.loader is None:
    raise RuntimeError("Cannot load _v969_strategy_core.py")
...
def build_unified_rating_shadow(
    ...
    for _, row in selected_policy_rows.sort_values("expert_id").iterrows():
        ...
        # V9.6.9 formalizes expert coverage as a request-level contract.
        ...
        if eid in reference_ids:
            ...
            if eid not in core.EXPERT_BY_ID:
                ...
                coverage.append({
                    "expert_id": eid, "expert": expert_name, "policy_key": policy_key,
                    "rating_engine": "V9.6.9_EXTENDED_ORIGINAL", "found_in_v961_reference": False,
                    "expert_found_in_rating_engine": False,
                    "policy_found_in_rating_engine": False, "raw_candidates": 0,
                    "selected_trades": 0, "signal_fingerprint": ""
                })
...
def attribution_self_test() -> None:
    ...
    print("V969_QUAD_TRACK_SELF_TEST_OK")
...
def statistical_self_test()->None:
    ...
    print("V969_STATISTICAL_VALIDATION_SELF_TEST_OK")
...
def shrinkage_allocation_self_test() -> None:
    ...
    assert TRACK["winner_selection_enabled"] is False and TRACK["seen_oos_must_not_select_winner"] is True
    print("V969_EVIDENCE_SHRINKAGE_AND_ALLOCATION_OK")
...
def original_gate_self_test() -> None:
    ...
    print("V969_ORIGINAL_SELECTION_AND_EXPERT_GATE_OK")
...
def rating_source_self_test() -> None:
    ...
    selftest_ref_request = ROOT / ".v969_selftest_reference_request.json"
    ...
    print("V969_UNIFIED_RATING_SOURCE_AND_DUAL_BUDGET_OK")
...
def main()->None:
    ...
    snapshot={"engine":ENGINE_NAME,"engine_version":"V9.6.9","seed_expert_id":seed_id,"seed_expert":expert.name,"policy_key":seed_key,"policy":asdict(policy),"feature_group":expert.feature_group,"feature_list":list(core.FEATURE_GROUPS[expert.feature_group]),"training_months":list(core.DEVELOPMENT_MONTHS),"diagnostic_oos_month":core.OOS_MONTH,"execution":{"fee_rate_per_side":core.FEE_RATE,"slippage_abs":core.SLIPPAGE_ABS,"next_bar_open":True,"same_candle_stop_before_tp":True},"model_runtime":REQUEST["model"],"statistical_validation":STAT,"code_hashes":{"entrypoint":file_sha256(ROOT/"autonomous_backtest_v9_6_9.py"),"strategy_core":file_sha256(ROOT/"_v969_strategy_core.py"),"base_engine":file_sha256(ROOT/"_v969_base_engine.py"),"request":file_sha256(REQUEST_PATH)},"official_data_fingerprint":data_fp,"signal_fingerprint":original_fp,"random_seed":REQUEST["model"]["base_seed"]}
    ...
    status.update({
        "engine": ENGINE_NAME,
        "release_version": "9.6.9",
        "release_type": "formal_stable_after_expert_coverage_failure",
        "qualified": False,
    ...
    report = f"""# BTCUSDT 5分钟 门控统计数据源统一与风险预算双轨 V9.6.9 报告
