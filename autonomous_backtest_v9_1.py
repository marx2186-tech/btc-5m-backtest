...
CACHE = ROOT / ".cache"
RESULTS = ROOT / "results_v9_1"
CACHE.mkdir(exist_ok=True)
RESULTS.mkdir(exist_ok=True)

ENGINE_VERSION = "V9.1"
ENGINE_NAME = "BTC 5m signal-funnel layered expert ensemble V9.1"
OOS_MONTH = "2026-06"
...
DEFAULT_REQUEST: dict[str, Any] = {
    ...
    "combination_beam_width": 240,
    "diagnostic_top_per_expert": 1,
...
def load_request() -> dict[str, Any]:
    ...
    path = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_1.json")))
    if path.exists():
        loaded = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise ValueError("request.v9_1.json must contain a JSON object")
...
if len(EVAL_MONTHS) != 2:
    raise ValueError("V9.1 requires exactly two evaluation months")
...
if str(last_eval) != OOS_MONTH:
    raise ValueError(f"V9.1 hard-codes {OOS_MONTH} as the untouched out-of-sample month")
...
if FEE_RATE != 0.0005:
    raise ValueError("V9.1 fixed requirement: fee_rate_per_side must be 0.0005")
if abs(SLIPPAGE_ABS - 0.2) > 1e-12:
    raise ValueError("V9.1 fixed requirement: each fill slippage must equal 0.2 USDT")
...
def add_features(
    ...
    # V9.1 signal funnel: 1h market state and 15m structure remain hard gates.
...
def fit_one_expert_model(
    ...
    min_rows = 45 if policy.expert_id in (0, 1) else 36
    min_class = 8 if policy.expert_id in (0, 1) else 6
    if len(y) < min_rows or len(np.unique(y)) < 2 or min(np.sum(y == 0), np.sum(y == 1)) < min_class:
        return None
...
def events_from_expert(
    ...
    eval_structural_mask = (bundle["months"] == eval_month) & (bundle["exits"] >= 0)
    train_prob = bundle["prob"][train_mask]
    min_training_candidates = 35 if policy.expert_id in (0, 1) else 28
    base_audit: dict[str, Any] = {
        "model_available": True,
        "training_candidates": int(len(train_prob)),
        "raw_structural_candidates": int(np.sum(eval_structural_mask)),
    }
    if len(train_prob) < min_training_candidates:
        return [], {
            **base_audit,
    ...
    keep_fraction = min(0.45, max(0.005, desired_total / len(train_prob)))
    ...
    eval_mask = eval_structural_mask & (bundle["prob"] >= probability_threshold)
    ...
    raw_after_probability = len(events)
    ...
    return events, {
        **base_audit,
        ...
        "raw_eval_after_probability": int(raw_after_probability),
...
def expert_candidate_score(candidate: ExpertCandidate) -> float:
    ...
    floor_wr = min(float(m["win_rate"]) for m in meaningful)
    ...
    total_raw = sum(raw_structural_candidates(candidate, m) for m in DEVELOPMENT_MONTHS)
    signal_coverage = sum(raw_structural_candidates(candidate, m) > 0 for m in DEVELOPMENT_MONTHS) / len(DEVELOPMENT_MONTHS)
    return float(
        floor_wr * 3200
        + min(floor_pf, 5.0) * 700
        + cumulative["win_rate"] * 1700
        + min(cumulative["profit_factor"], 6.0) * 500
        + cumulative["net_R"] * 28
        - cumulative["max_drawdown_R"] * 22
        - target_gap * 18
        - consecutive_zero_opportunity_months(candidate) * 700
        + min(total_raw, 240) * 2.0
        + signal_coverage * 500
    )
...
def combination_score(candidate: CombinationCandidate) -> float:
    ...
    median_wr = float(np.median([m["win_rate"] for m in meaningful]))
    ...
    count_penalty = sum(
        abs(int(m["trades"]) - np.clip(int(m["trades"]), max(8, MIN_TRADES - 7), MAX_TRADES + 8))
        for m in stats
    )
...
def policy_grid_for_expert(expert_id: int) -> list[ExpertPolicy]:
    if expert_id == 1:
        targets, probs, pcts, cooldowns = (8, 12, 16), (0.48, 0.52, 0.56), (0.70, 0.78, 0.85), (1, 3, 6)
    elif expert_id == 0:
        targets, probs, pcts, cooldowns = (8, 12, 16), (0.48, 0.52, 0.56), (0.70, 0.78, 0.85), (1, 3, 6)
    elif expert_id in (2, 3):
        targets, probs, pcts, cooldowns = (5, 8, 12), (0.48, 0.52, 0.56), (0.70, 0.78, 0.85), (2, 5, 8)
    else:
        targets, probs, pcts, cooldowns = (4, 7, 10), (0.49, 0.53, 0.57), (0.72, 0.80, 0.87), (2, 5, 8)
    return [
    ...
    for expert_id, candidates in all_candidates.items():
        for rank, candidate in enumerate(candidates[:30], start=1):
            ...
            for month, value in candidate.monthly_metrics.items():
                ...
                audit = candidate.monthly_thresholds.get(month, {})
                for key in (
                    "model_available", "training_candidates", "raw_structural_candidates",
                    "probability_threshold", "raw_eval_after_probability",
...
def self_test() -> None:
    ...
    if x.empty:
        raise RuntimeError("V9.1 self-test produced no feature rows")
    missing = [feature for feature in FEATURES if feature not in x.columns]
    if missing:
        raise RuntimeError(f"V9.1 self-test missing features: {missing}")
    ...
    if x_gap.empty:
        raise RuntimeError("V9.1 missing-auxiliary regression produced no rows")
    ...
    if set(experts) != set(range(6)):
        raise RuntimeError("V9.1 expert registry mismatch")
    ...
    if not funnel or not all(EXPERT_NAME[i] in funnel[next(iter(funnel))] for i in range(6)):
        raise RuntimeError("V9.1 signal funnel registry failed")
    if build_combinations({}) != []:
        raise RuntimeError("V9.1 empty combination must not be treated as a valid policy")
    ...
    if consecutive_zero_opportunity_months(probe) != 0:
        raise RuntimeError("V9.1 no-opportunity month was incorrectly treated as failure")
    if alignment_audit["eth_coverage_after_past_fill"] < 0.99:
        raise RuntimeError("Synthetic alignment unexpectedly failed")
    print(
        f"V9.1 self-test passed: rows={len(x)}, features={len(FEATURES)}, "
...
def main() -> None:
    ...
    status = {
        ...
        "engine": ENGINE_NAME,
        "method": "Sep-Nov initial train; Dec-May rolling funnel diagnostics, layered expert elimination and policy selection; June evaluated once after freeze",
        "architecture": "completed 1h market state → completed 15m structure → broader 5m candidate → expert model → next 5m open fill",
        "symbol": SYMBOL,
        "interval": INTERVAL,
        "months": list(MONTHS),
        "oos_month": OOS_MONTH,
        ...
        "signal_funnel_file": "results_v9_1/signal_funnel.csv",
    ...
    report = f"""# BTCUSDT 5分钟 信号漏斗与分层淘汰 V9.1 回测报告
