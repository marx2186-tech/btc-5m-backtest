...
CACHE = ROOT / ".cache"
RESULTS = ROOT / "results_v9_2"
CACHE.mkdir(exist_ok=True)
RESULTS.mkdir(exist_ok=True)

ENGINE_VERSION = "V9.2"
ENGINE_NAME = "BTC 5m cross-month calibrated expert ensemble V9.2"
OOS_MONTH = "2026-06"
...
def load_request() -> dict[str, Any]:
    ...
    path = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_2.json")))
    if path.exists():
        loaded = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise ValueError("request.v9_2.json must contain a JSON object")
...
if len(EVAL_MONTHS) != 2:
    raise ValueError("V9.2 requires exactly two evaluation months")
...
if str(last_eval) != OOS_MONTH:
    raise ValueError(f"V9.2 hard-codes {OOS_MONTH} as the untouched out-of-sample month")
...
if FEE_RATE != 0.0005:
    raise ValueError("V9.2 fixed requirement: fee_rate_per_side must be 0.0005")
if abs(SLIPPAGE_ABS - 0.2) > 1e-12:
    raise ValueError("V9.2 fixed requirement: each fill slippage must equal 0.2 USDT")
    ...
    for attempt in range(attempts):
        try:
            response = requests.get(
                url,
                timeout=90,
                headers={"User-Agent": "btc-cross-month-calibrated-v9-2/1.0"},
            )
...
def add_features(
    ...
    # V9.2 signal funnel: 1h market state and 15m structure remain hard gates.
...
def policy_grid_for_expert(expert_id: int) -> list[ExpertPolicy]:
    ...
    # by V9.1.
    if expert_id in (0, 1):
        targets, probs, pcts, cooldowns = (6, 10, 14), (0.34, 0.38, 0.42), (0.65, 0.75, 0.85), (1, 3, 6)
...
def self_test() -> None:
    ...
    if x.empty:
        raise RuntimeError("V9.2 self-test produced no feature rows")
    missing = [feature for feature in FEATURES if feature not in x.columns]
    if missing:
        raise RuntimeError(f"V9.2 self-test missing features: {missing}")
    ...
    if x_gap.empty:
        raise RuntimeError("V9.2 missing-auxiliary regression produced no rows")
    ...
    if set(experts) != set(range(6)):
        raise RuntimeError("V9.2 expert registry mismatch")
    ...
    if not funnel or not all(EXPERT_NAME[i] in funnel[next(iter(funnel))] for i in range(6)):
        raise RuntimeError("V9.2 signal funnel registry failed")
    if build_combinations({}) != []:
        raise RuntimeError("V9.2 empty combination must not be treated as a valid policy")
    ...
    if consecutive_zero_opportunity_months(probe) != 0:
        raise RuntimeError("V9.2 no-opportunity month was incorrectly treated as failure")
    if alignment_audit["eth_coverage_after_past_fill"] < 0.99:
        raise RuntimeError("Synthetic alignment unexpectedly failed")
    print(
        f"V9.2 self-test passed: rows={len(x)}, features={len(FEATURES)}, "
...
def main() -> None:
    ...
    all_trades = pd.concat(nonempty, ignore_index=True) if nonempty else pd.DataFrame(columns=[
        ...
        "calibrated_probability", "online_score_percentile", "entry", "risk_abs", "target_R", "net_R", "win", "exit_reason", "bars",
    ])
    ...
    status = {
        ...
        "engine": ENGINE_NAME,
        "method": "Rolling core-train plus latest-complete-month holdout calibration; Dec-May cross-month stability selection; June evaluated once after freeze",
        "architecture": "completed 1h market state → completed 15m structure → broader 5m candidate → holdout-calibrated model → online past-score rank → next 5m open fill",
        "symbol": SYMBOL,
        "interval": INTERVAL,
        "months": list(MONTHS),
        "oos_month": OOS_MONTH,
        ...
        "signal_funnel_file": "results_v9_2/signal_funnel.csv",
    ...
    report = f"""# BTCUSDT 5分钟 跨月稳定与概率校准 V9.2 回测报告
