...
RESULTS = ROOT / "results_v9_7_1"
REQUEST_PATH = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_7_1.json")))
...
BLIND = REQUEST["blind_evidence"]
ENGINE_NAME = "BTC 5m fixed-cutoff partial blind window through 2026-07-27 V9.7.1"

spec = importlib.util.spec_from_file_location("v971_strategy_core", ROOT / "_v971_strategy_core.py")
if spec is None or spec.loader is None:
    raise RuntimeError("Cannot load _v971_strategy_core.py")
...
def build_unified_rating_shadow(
    ...
    for _, row in selected_policy_rows.sort_values("expert_id").iterrows():
        ...
        # V9.7.1 formalizes expert coverage as a request-level contract.
...
def attribution_self_test() -> None:
    ...
    print("V971_QUAD_TRACK_SELF_TEST_OK")
...
def statistical_self_test()->None:
    ...
    print("V971_STATISTICAL_VALIDATION_SELF_TEST_OK")
...
def shrinkage_allocation_self_test() -> None:
    ...
    assert TRACK["winner_selection_enabled"] is False and TRACK["seen_oos_must_not_select_winner"] is True
    print("V971_EVIDENCE_SHRINKAGE_AND_ALLOCATION_OK")
...
def original_gate_self_test() -> None:
    ...
    print("V971_ORIGINAL_SELECTION_AND_EXPERT_GATE_OK")
...
def rating_source_self_test() -> None:
    ...
    selftest_ref_request = ROOT / ".v971_selftest_reference_request.json"
    ...
    print("V971_UNIFIED_RATING_SOURCE_AND_DUAL_BUDGET_OK")




BLIND_LEDGER_COLUMNS = [
...
def _calendar_latest_complete_month(now: pd.Timestamp | None = None) -> str:
    ...
    grace = int(BLIND.get("archive_grace_days", 5))
...
def _merge_append_only(previous: pd.DataFrame, current: pd.DataFrame, ledger_name: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if previous.empty and len(previous.columns)==0: previous=pd.DataFrame(columns=BLIND_LEDGER_COLUMNS)
    if current.empty and len(current.columns)==0: current=pd.DataFrame(columns=BLIND_LEDGER_COLUMNS)
    ...
    immutable = list(BLIND.get("immutable_event_fields", []))
    ...
    if mutations:
        raise RuntimeError(f"Append-only blind ledger mutation in {ledger_name}: {mutations[:5]}")
...
def _with_blind_months(mod: Any, months: list[str]):
    class Ctx:
        def __enter__(self_nonlocal):
            ...
            mod.MONTHS=tuple(months); mod.OOS_MONTH="__BLIND_CURRENT_MONTH_GUARD__"
...
def _window_start() -> pd.Timestamp:
    return pd.Timestamp(str(BLIND["window_start_utc"])).tz_convert("UTC")


def _window_cutoff() -> pd.Timestamp:
    return pd.Timestamp(str(BLIND["fixed_cutoff_utc"])).tz_convert("UTC")
...
def _kline_rest_day(symbol: str, data_type: str, day: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    start_ms = int(pd.Timestamp(f"{day}T00:00:00Z").timestamp() * 1000)
    end_ms = start_ms + 86_400_000 - 1
    endpoint = "klines" if data_type == "klines" else "premiumIndexKlines"
    cache = core.base.CACHE / f"rest-{endpoint}-{symbol}-{core.INTERVAL}-{day}.json"
    if cache.exists() and cache.stat().st_size:
        payload = json.loads(cache.read_text(encoding="utf-8"))
    else:
        response = requests.get(
            f"https://fapi.binance.com/fapi/v1/{endpoint}",
            params={"symbol": symbol, "interval": core.INTERVAL, "startTime": start_ms, "endTime": end_ms, "limit": 1500},
            timeout=45,
            headers={"User-Agent": "btc-5m-v9.7.1-fixed-cutoff/1.0"},
        )
        response.raise_for_status()
        payload = response.json()
        cache.write_text(json.dumps(payload, ensure_ascii=False), encoding="utf-8")
    frame = pd.DataFrame(payload)
    if frame.empty or frame.shape[1] < 12:
        raise RuntimeError(f"Official Binance REST {endpoint} returned no usable rows for {symbol} {day}")
    frame = frame.iloc[:, :12]
    frame.columns = core.base.COLS
    for col in [c for c in core.base.COLS if c != "ignore"]:
        frame[col] = pd.to_numeric(frame[col], errors="coerce")
    return frame, {"source": f"Binance USD-M Futures official REST {endpoint}", "day": day, "rows": int(len(frame))}


def _daily_kline_archive(symbol: str, data_type: str, dates: list[str]) -> tuple[pd.DataFrame, dict[str, Any]]:
    frames: list[pd.DataFrame] = []
    files: list[dict[str, Any]] = []
    fallbacks: list[dict[str, Any]] = []
    for day in dates:
        name = f"{symbol}-{core.INTERVAL}-{day}.zip"
        base_url = f"https://data.binance.vision/data/futures/um/daily/{data_type}/{symbol}/{core.INTERVAL}"
        cache_name = f"daily-{data_type}-{name}"
        try:
            raw, digest = core.base.read_verified_zip(
                f"{base_url}/{name}", f"{base_url}/{name}.CHECKSUM", cache_name
            )
            frame = core.base.parse_kline_csv(core.base.read_single_csv_zip(raw, name))
            source_row = {"file": name, "sha256": digest, "rows": int(len(frame)), "source": "verified_daily_archive"}
        except Exception as exc:
            frame, fallback = _kline_rest_day(symbol, data_type, day)
            fallback["archive_error"] = f"{type(exc).__name__}: {exc}"[:500]
            fallbacks.append(fallback)
            source_row = {"file": name, "sha256": None, "rows": int(len(frame)), "source": "official_rest_fallback"}
        start_ms = int(pd.Timestamp(f"{day}T00:00:00Z").timestamp() * 1000)
        end_ms = start_ms + 86_400_000
        frame = frame[(frame["open_time"] >= start_ms) & (frame["open_time"] < end_ms)].copy()
        expected = np.arange(start_ms, end_ms, core.base.STEP_MS, dtype=np.int64)
        actual = np.sort(frame["open_time"].astype("int64").unique())
        if len(frame) != len(expected) or not np.array_equal(actual, expected):
            raise RuntimeError(
                f"Incomplete official daily {data_type}/{symbol} data {day}: "
                f"rows={len(frame)} expected={len(expected)}"
            )
        frames.append(frame)
        files.append(source_row)
    data = pd.concat(frames, ignore_index=True).sort_values("open_time").drop_duplicates("open_time") if frames else pd.DataFrame(columns=core.base.COLS)
    return data.reset_index(drop=True), {
        "source": "Binance verified daily archive with official REST fallback",
        "symbol": symbol,
        "data_type": data_type,
        "dates": dates,
        "files": files,
        "rest_fallbacks": fallbacks,
        "rows": int(len(data)),
    }


def _parse_funding_content(content: bytes, source_name: str) -> pd.DataFrame:
    ...
    if time_col is None or rate_col is None:
        if frame.shape[1] < 2:
            raise RuntimeError(f"Unexpected funding-rate schema for {source_name}: {list(frame.columns)}")
...
def _funding_rest_fallback(dates: list[str]) -> tuple[pd.DataFrame, dict[str, Any]]:
    start_ms = int(pd.Timestamp(f"{dates[0]}T00:00:00Z").timestamp() * 1000)
    end_ms = int((pd.Timestamp(f"{dates[-1]}T00:00:00Z") + pd.Timedelta(days=1)).timestamp() * 1000) - 1
    cache = core.base.CACHE / f"funding-rest-{core.SYMBOL}-{dates[0]}-{dates[-1]}.json"
    if cache.exists() and cache.stat().st_size:
        payload = json.loads(cache.read_text(encoding="utf-8"))
    else:
        response = requests.get(
            "https://fapi.binance.com/fapi/v1/fundingRate",
            params={"symbol": core.SYMBOL, "startTime": start_ms, "endTime": end_ms, "limit": 1000},
            timeout=45,
            headers={"User-Agent": "btc-5m-v9.7.1-fixed-cutoff/1.0"},
        )
        response.raise_for_status()
        ...
        cache.write_text(json.dumps(payload, ensure_ascii=False), encoding="utf-8")
    frame = pd.DataFrame(payload)
    if frame.empty or "fundingTime" not in frame or "fundingRate" not in frame:
        raise RuntimeError("Official Binance funding-rate REST fallback returned no usable rows")
    out = pd.DataFrame({
        "calc_time": pd.to_numeric(frame["fundingTime"], errors="coerce"),
        "funding_rate": pd.to_numeric(frame["fundingRate"], errors="coerce"),
    }).dropna()
    return out, {"source": "Binance USD-M Futures official REST fundingRate", "rows": int(len(out)), "fallback_used": True}


def _daily_funding_archive(dates: list[str]) -> tuple[pd.DataFrame, dict[str, Any]]:
    frames: list[pd.DataFrame] = []
    files: list[dict[str, Any]] = []
    errors: list[str] = []
    for day in dates:
        name = f"{core.SYMBOL}-fundingRate-{day}.zip"
        base_url = f"https://data.binance.vision/data/futures/um/daily/fundingRate/{core.SYMBOL}"
        try:
            raw, digest = core.base.read_verified_zip(
                f"{base_url}/{name}", f"{base_url}/{name}.CHECKSUM", f"daily-funding-{name}"
            )
            out = _parse_funding_content(core.base.read_single_csv_zip(raw, name), name)
            frames.append(out)
            files.append({"file": name, "sha256": digest, "rows": int(len(out))})
        except Exception as exc:
            errors.append(f"{day}: {type(exc).__name__}: {exc}"[:500])
            break
    if errors:
        out, audit = _funding_rest_fallback(dates)
        audit["daily_archive_errors"] = errors
        return out, audit
    data = pd.concat(frames, ignore_index=True).sort_values("calc_time").drop_duplicates("calc_time") if frames else pd.DataFrame(columns=["calc_time", "funding_rate"])
    return data.reset_index(drop=True), {
        "source": "Binance verified daily fundingRate archives",
        "files": files,
        "rows": int(len(data)),
        "fallback_used": False,
    }
...
def _combine_and_audit_klines(monthly: pd.DataFrame, daily: pd.DataFrame, label: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    data = pd.concat([monthly, daily], ignore_index=True).sort_values("open_time").drop_duplicates("open_time", keep="last").reset_index(drop=True)
    ...
    passed = bool(len(data)==len(expected) and len(missing)==0 and len(extra)==0 and pd.Series(actual).duplicated().sum()==0)
    audit = {
        "label": label,
        ...
        "duplicate_timestamps": int(pd.Series(actual).duplicated().sum()),
        "passed": passed,
    }
    if not passed:
        raise RuntimeError(f"V9.7.1 fixed-cutoff kline audit failed for {label}: {json.dumps(audit, ensure_ascii=False)}")
    return data, audit


def _load_fixed_cutoff_frames(ref: Any) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    dates = _daily_window_dates()
    monthly_end = str(pd.Period(_window_start().strftime("%Y-%m"), freq="M") - 1)
    monthly_months = _periods("2025-01", monthly_end)
    ...
    try:
        raw_monthly, raw_monthly_audit = core.base.load_official_data()
        eth_monthly, eth_monthly_audit = core.base.load_auxiliary_kline("ETHUSDT", "klines")
        premium_monthly, premium_monthly_audit = core.base.load_auxiliary_kline(core.SYMBOL, "premiumIndexKlines")
        funding_monthly, funding_monthly_audit = core.base.load_funding_rate()
    finally:
        core.base.MONTHS = old_core_months
        ref.base.MONTHS = old_ref_months

    raw_daily, raw_daily_audit = _daily_kline_archive(core.SYMBOL, "klines", dates)
    eth_daily, eth_daily_audit = _daily_kline_archive("ETHUSDT", "klines", dates)
    premium_daily, premium_daily_audit = _daily_kline_archive(core.SYMBOL, "premiumIndexKlines", dates)
    funding_daily, funding_daily_audit = _daily_funding_archive(dates)

    raw, raw_audit = _combine_and_audit_klines(raw_monthly, raw_daily, "BTCUSDT_5m")
    eth, eth_audit = _combine_and_audit_klines(eth_monthly, eth_daily, "ETHUSDT_5m")
    premium, premium_audit = _combine_and_audit_klines(premium_monthly, premium_daily, "BTCUSDT_premiumIndexKlines_5m")
    funding = pd.concat([funding_monthly, funding_daily], ignore_index=True).sort_values("calc_time").drop_duplicates("calc_time", keep="last").reset_index(drop=True)
    ...
    if not x.index.equals(rx.index):
        raise RuntimeError("V9.7.1 fixed-cutoff feature-frame index mismatch")
    if x.empty:
        raise RuntimeError("V9.7.1 fixed-cutoff feature frame is empty")
    ...
    if feature_tail_gap < pd.Timedelta(0) or feature_tail_gap > pd.Timedelta(minutes=60):
        raise RuntimeError(
            f"Fixed-cutoff usable feature frame ended at {x.index[-1]}; "
            f"allowed tail gap is 0-60 minutes from {target_last_bar}"
        )
    return rx, x, {
        "window_type": BLIND["window_type"],
        "window_start_utc": _window_start().isoformat(),
        "fixed_cutoff_utc": _window_cutoff().isoformat(),
        ...
        "official_monthly": raw_monthly_audit,
        "eth_monthly": eth_monthly_audit,
        "premium_monthly": premium_monthly_audit,
        "funding_monthly": funding_monthly_audit,
        "official_daily": raw_daily_audit,
        "eth_daily": eth_daily_audit,
        "premium_daily": premium_daily_audit,
        "funding_daily": funding_daily_audit,
        "official_combined_audit": raw_audit,
        "eth_combined_audit": eth_audit,
        "premium_combined_audit": premium_audit,
        "funding_combined_rows": int(len(funding)),
        "extended_alignment": align,
        "reference_alignment": ralign,
    }

def _blind_selected_for_experts(ref: Any, rx: pd.DataFrame, x: pd.DataFrame, tiers: pd.DataFrame, blind_months: list[str], expanded_months: list[str]) -> pd.DataFrame:
    frames=[]
    ...
    for _,row in tiers.iterrows():
        ...
        selected["lane"]="V9.7.1_BLIND_NO_MONTHLY_BUDGET_ORIGINAL"; selected["rating_engine"]="V9.6.1_REFERENCE" if mod is ref else "V9.6.9_FROZEN_EXTENDED_ORIGINAL"
        selected=_add_event_times(selected,frame); frames.append(selected)
    return pd.concat(frames,ignore_index=True,sort=False) if frames else pd.DataFrame()


def _blind_seed_risk_lanes(ref: Any, rx: pd.DataFrame, tiers: pd.DataFrame, blind_months: list[str], expanded_months: list[str]) -> pd.DataFrame:
...
def _blind_outputs(previous_expert: pd.DataFrame, previous_risk: pd.DataFrame, ref: Any, tiers: pd.DataFrame, rating_shadow: pd.DataFrame, dev_months: list[str]) -> dict[str,Any]:
    start_month = str(BLIND["start_month"])
    window_months = [start_month]
    rx, x, data_audit = _load_fixed_cutoff_frames(ref)
    expanded = _periods("2025-01", start_month)

    registry = [{
        "month": start_month,
        "state": "FIXED_PARTIAL_CUTOFF_EVALUATED",
        "calendar_complete": False,
        "archive_available": True,
        "window_type": str(BLIND["window_type"]),
        "window_start_utc": _window_start().isoformat(),
    ...
    pd.DataFrame(registry).to_csv(RESULTS/"blind_month_registry.csv", index=False)

    new_expert = _blind_selected_for_experts(ref, rx, x, tiers, window_months, expanded)
    new_risk = _blind_seed_risk_lanes(ref, rx, tiers, window_months, expanded)
    cutoff = _window_cutoff()
    for label, frame in (("expert", new_expert), ("risk", new_risk)):
        if not frame.empty:
            exit_times = pd.to_datetime(frame["exit_time_utc"], utc=True, errors="coerce")
            if exit_times.isna().any() or bool((exit_times > cutoff).any()):
                raise RuntimeError(f"{label} blind ledger contains an unresolved or post-cutoff exit")

    expert_ledger, expert_merge = _merge_append_only(previous_expert, new_expert, "blind_expert_trade_ledger")
    risk_ledger, risk_merge = _merge_append_only(previous_risk, new_risk, "blind_risk_budget_trade_ledger")
    expert_ledger.to_csv(RESULTS/"blind_expert_trade_ledger.csv", index=False)
    risk_ledger.to_csv(RESULTS/"blind_risk_budget_trade_ledger.csv", index=False)

    # Provisional statistics may include this fixed partial window, but activation stays locked.
    provisional = []
    evidence_months = dev_months + ["2026-06", start_month]
    for _, row in tiers.iterrows():
        eid = int(row["expert_id"]); key = str(row["policy_key"])
        dev = rating_shadow[
            (rating_shadow.get("expert_id", pd.Series(dtype=int)).astype(int) == eid)
            & (rating_shadow.get("policy_key", pd.Series(dtype=str)).astype(str) == key)
            & (rating_shadow.get("month", pd.Series(dtype=str)).astype(str).isin(dev_months))
        ].copy() if not rating_shadow.empty else pd.DataFrame()
        blind = expert_ledger[
            (expert_ledger.get("expert_id", pd.Series(dtype=int)).astype(int) == eid)
            & (expert_ledger.get("policy_key", pd.Series(dtype=str)).astype(str) == key)
        ].copy() if not expert_ledger.empty else pd.DataFrame()
        combined = pd.concat([dev, blind], ignore_index=True, sort=False)
        summary, _, _ = evidence_for_expert(combined, evidence_months, bool(row.get("selected_cluster_robust", False)))
        provisional_tier, reasons = tier_for(summary)
        blind_trades = int(len(blind))
        provisional.append({
            "expert_id": eid,
            "expert": row["expert"],
            "policy_key": key,
            "frozen_tier": row["tier"],
            "provisional_tier": provisional_tier,
            "blind_trades": blind_trades,
            "complete_blind_months": 0,
            "fixed_partial_windows": 1,
            "window_start_utc": _window_start().isoformat(),
            "window_end_utc": _window_cutoff().isoformat(),
            "watch_evidence_ready": False,
            "qualified_evidence_ready": False,
            "activation_ready_for_review": False,
            "automatic_activation": False,
            "rejection_reason": "|".join(reasons),
            **summary,
        })
    provisional_df = pd.DataFrame(provisional)
    provisional_df.to_csv(RESULTS/"blind_provisional_tier_audit.csv", index=False)
    ...
    for lane in ["V9.6.1_MONTHLY_BUDGET", "V9.6.7_NO_MONTHLY_BUDGET"]:
        ...
        metrics = base_metrics(q)
        lane_progress[lane] = {
            **metrics,
            "complete_calendar_months": 0,
            "fixed_partial_windows": 1,
            "calendar_requirement_met": False,
            "trade_requirement_met": metrics["trades"] >= int(BLIND["minimum_trades_per_risk_lane"]),
            "partial_window_diagnostic_ready": metrics["trades"] > 0,
            "evidence_ready": False,
        }

    partial_trade_review_ready = all(v["trade_requirement_met"] for v in lane_progress.values())
    progress = {
        "blind_start_month": start_month,
        "window_type": str(BLIND["window_type"]),
        "window_start_utc": _window_start().isoformat(),
        "fixed_cutoff_utc": _window_cutoff().isoformat(),
        "window_evaluated": True,
        "partial_window_counts_as_complete_month": False,
        "complete_blind_months": [],
        "complete_blind_month_count": 0,
        "fixed_partial_window_count": 1,
        "latest_available_archive_month": start_month,
        "risk_lanes": lane_progress,
        "partial_trade_review_ready": partial_trade_review_ready,
        "decision_ready": False,
        "winner_selection_enabled": False,
        "winner_decision": "FIXED_PARTIAL_WINDOW_DIAGNOSTIC_ONLY",
        "automatic_tier_activation_enabled": False,
    }
    (RESULTS/"blind_evidence_progress.json").write_text(json.dumps(progress, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    readiness = {
        "decision_ready": False,
        "window_type": str(BLIND["window_type"]),
        "partial_window_review_ready": partial_trade_review_ready,
        "requirements": {
            "minimum_complete_calendar_months_for_formal_decision": BLIND["minimum_complete_calendar_months"],
            "minimum_trades_per_risk_lane": BLIND["minimum_trades_per_risk_lane"],
        },
        "winner_selection_enabled": False,
        "action": "review partial-window diagnostics without selecting or tuning",
    }
    (RESULTS/"blind_decision_readiness.json").write_text(json.dumps(readiness, ensure_ascii=False, indent=2), encoding="utf-8")
    fixed_summary = {
        "window_type": str(BLIND["window_type"]),
        "window_start_utc": _window_start().isoformat(),
        "window_end_utc": _window_cutoff().isoformat(),
        "days_included": len(_daily_window_dates()),
        "last_raw_5m_bar_utc": data_audit["last_raw_5m_bar_utc"],
        ...
        "expert_shadow_trades": int(len(expert_ledger)),
        "risk_budget_trades": int(len(risk_ledger)),
        "right_censored_trades_excluded": bool(BLIND.get("exclude_right_censored_trades", True)),
        "partial_window_counts_as_complete_month": False,
    }
    (RESULTS/"blind_fixed_window_summary.json").write_text(json.dumps(fixed_summary, ensure_ascii=False, indent=2), encoding="utf-8")
    integrity = {
        "append_only_enabled": True,
        "expert_ledger": expert_merge,
        "risk_budget_ledger": risk_merge,
        "fixed_cutoff_data_audit": data_audit,
        "window_start_utc": _window_start().isoformat(),
        "fixed_cutoff_utc": _window_cutoff().isoformat(),
        "all_recorded_exits_at_or_before_cutoff": True,
        "right_censored_trades_excluded": bool(BLIND.get("exclude_right_censored_trades", True)),
        "blind_data_used_for_policy_search": False,
        "blind_data_used_for_threshold_tuning": False,
        "blind_data_used_for_signal_formula_changes": False,
        "frozen_baseline_cutoff": REQUEST["release"]["frozen_baseline_cutoff"],
    }
    (RESULTS/"blind_integrity_audit.json").write_text(json.dumps(integrity, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    return {"progress": progress, "integrity": integrity, "provisional": provisional_df}

def blind_accumulation_self_test() -> None:
    assert _periods("2026-07", "2026-06") == []
    assert _periods("2026-07", "2026-09") == ["2026-07", "2026-08", "2026-09"]
    assert _window_start() == pd.Timestamp("2026-07-01T00:00:00Z")
    assert _window_cutoff() == pd.Timestamp("2026-07-27T23:59:59Z")
    assert len(_daily_window_dates()) == 27
    assert _daily_window_dates()[0] == "2026-07-01"
    assert _daily_window_dates()[-1] == "2026-07-27"
    assert BLIND["window_type"] == "FIXED_PARTIAL_CUTOFF"
    assert BLIND["partial_window_counts_as_complete_month"] is False
    start_ms = int(pd.Timestamp("2025-01-01T00:00:00Z").timestamp() * 1000)
    last_ms = int(pd.Timestamp("2026-07-27T23:55:00Z").timestamp() * 1000)
    times = np.arange(start_ms, last_ms + core.base.STEP_MS, core.base.STEP_MS, dtype=np.int64)
    synthetic = pd.DataFrame({"open_time": times})
    split_ms = int(pd.Timestamp("2026-07-01T00:00:00Z").timestamp() * 1000)
    combined, cutoff_audit = _combine_and_audit_klines(
        synthetic[synthetic["open_time"] < split_ms],
        synthetic[synthetic["open_time"] >= split_ms],
        "SELF_TEST",
    )
    assert cutoff_audit["passed"] and len(combined) == len(times)
    funding_sample = b"calc_time,last_funding_rate\n1782864000000,0.0001\n"
    parsed_funding = _parse_funding_content(funding_sample, "self-test")
    assert len(parsed_funding) == 1 and abs(float(parsed_funding.iloc[0]["funding_rate"]) - 0.0001) < 1e-12
    base_row={"lane":"A","expert_id":21,"policy_key":"p","signal_time_utc":"2026-07-01T00:00:00+00:00","entry_time_utc":"2026-07-01T00:05:00+00:00","exit_time_utc":"2026-07-01T01:00:00+00:00","direction":-1,"net_r":1.2,"win":True,"month":"2026-07"}
    old=pd.DataFrame([base_row]);same=pd.DataFrame([dict(base_row)]);new=pd.DataFrame([dict(base_row,signal_time_utc="2026-07-02T00:00:00+00:00")])
    merged,audit=_merge_append_only(old,same,"test")
    assert len(merged)==1 and audit["append_only_pass"] and audit["unchanged_rows"]==1
    merged2,audit2=_merge_append_only(old,new,"test")
    assert len(merged2)==2 and audit2["new_rows"]==1
    bad=pd.DataFrame([dict(base_row,net_r=-1.0)])
    try:
        _merge_append_only(old,bad,"test")
    except RuntimeError:
        pass
    else:
        raise AssertionError("append-only mutation was not blocked")
    assert BLIND["winner_selection_enabled"] is False
    assert BLIND["automatic_tier_activation_enabled"] is False
    assert int(BLIND["minimum_complete_calendar_months"]) == 3
    print("V971_FIXED_CUTOFF_APPEND_ONLY_SELF_TEST_OK")


def main()->None:
    previous_expert_ledger = _read_csv_or_empty(RESULTS / "blind_expert_trade_ledger.csv", BLIND_LEDGER_COLUMNS)
    previous_risk_ledger = _read_csv_or_empty(RESULTS / "blind_risk_budget_trade_ledger.csv", BLIND_LEDGER_COLUMNS)
    core.main()
    ...
    if not x.index.equals(rx.index):
        raise RuntimeError(
            "V9.7.1 rating feature-frame index mismatch: "
    ...
    # V9.7.1 invariant: final expert evidence is rebuilt from the complete no-monthly-budget
    ...
    snapshot={"engine":ENGINE_NAME,"engine_version":"V9.7.1","seed_expert_id":seed_id,"seed_expert":expert.name,"policy_key":seed_key,"policy":asdict(policy),"feature_group":expert.feature_group,"feature_list":list(core.FEATURE_GROUPS[expert.feature_group]),"training_months":list(core.DEVELOPMENT_MONTHS),"diagnostic_oos_month":core.OOS_MONTH,"execution":{"fee_rate_per_side":core.FEE_RATE,"slippage_abs":core.SLIPPAGE_ABS,"next_bar_open":True,"same_candle_stop_before_tp":True},"model_runtime":REQUEST["model"],"statistical_validation":STAT,"code_hashes":{"entrypoint":file_sha256(ROOT/"autonomous_backtest_v9_7_1.py"),"strategy_core":file_sha256(ROOT/"_v971_strategy_core.py"),"base_engine":file_sha256(ROOT/"_v971_base_engine.py"),"request":file_sha256(REQUEST_PATH)},"official_data_fingerprint":data_fp,"signal_fingerprint":original_fp,"random_seed":REQUEST["model"]["base_seed"]}
    ...
    if bool(EXPERT_COVERAGE.get("require_complete_expert_coverage", True)) and not expert_id_coverage_complete:
        ...
        raise RuntimeError(f"V9.7.1 expert coverage incomplete: missing={missing_ids}, extra={extra_ids}")
    if bool(EXPERT_COVERAGE.get("require_complete_policy_coverage", True)) and not policy_coverage_complete:
        raise RuntimeError("V9.7.1 policy coverage incomplete; inspect rating_source_policy_coverage.csv")
    if not feature_frame_coverage_complete:
        ...
        raise RuntimeError(f"V9.7.1 feature-frame coverage incomplete: {bad}")
    rating_audit={
        "final_rating_source":str(RATING_SOURCE["source_lane"]),
    ...
    blind_result = _blind_outputs(previous_expert_ledger, previous_risk_ledger, ref, new_tiers, rating_shadow, dev_months)
    ...
    status.update({
        "engine": ENGINE_NAME,
        "release_version": "9.7.1",
        "release_type": "fixed_cutoff_partial_blind_window",
        "qualified": False,
        ...
        "blind_evidence_complete": bool(blind_result["progress"]["decision_ready"]),
        "blind_start_month": str(BLIND["start_month"]),
        "blind_window_type": str(BLIND["window_type"]),
        "blind_window_start_utc": str(BLIND["window_start_utc"]),
        "blind_fixed_cutoff_utc": str(BLIND["fixed_cutoff_utc"]),
        "blind_partial_window_evaluated": bool(blind_result["progress"]["window_evaluated"]),
        "blind_complete_month_count": int(blind_result["progress"]["complete_blind_month_count"]),
        "blind_fixed_partial_window_count": int(blind_result["progress"]["fixed_partial_window_count"]),
        "blind_latest_available_archive_month": blind_result["progress"]["latest_available_archive_month"],
        "blind_winner_selection_enabled": False,
        "blind_automatic_tier_activation_enabled": False,
        "constraints": {
    ...
    report = f"""# BTCUSDT 5分钟 固定截点部分月盲测 V9.7.1 报告

- 架构：完整复现V9.6.9冻结基线 → 使用官方月度数据到2026年6月 → 追加官方日度数据至2026年7月27日 → 两条风险预算并行诊断 → 不选胜者、不自动晋级。
...
- 胜出通道选择：**关闭**。2026年6月以前全部冻结；本版只评估2026年7月1日至7月27日固定部分月窗口。
- 实盘资格：**不合格**。
...
| 指标 | 结果 |
|---|---:|
| 窗口类型 | {BLIND['window_type']} |
| 起始时间 | {BLIND['window_start_utc']} |
| 截止时间 | {BLIND['fixed_cutoff_utc']} |
| 纳入自然日 | {len(_daily_window_dates())} |
| 最后一根原始K线 | {blind_result['integrity']['fixed_cutoff_data_audit']['last_raw_5m_bar_utc']} |
| 最后一根可用特征K线 | {blind_result['integrity']['fixed_cutoff_data_audit']['last_usable_feature_bar_utc']} |
| 特征尾部缺口 | {blind_result['integrity']['fixed_cutoff_data_audit']['feature_tail_gap_minutes']:.0f}分钟 |
| 完整自然月数量 | {blind_result['progress']['complete_blind_month_count']} |
| 月度预算盲测交易 | {blind_result['progress']['risk_lanes']['V9.6.1_MONTHLY_BUDGET']['trades']} |
| 无月度预算盲测交易 | {blind_result['progress']['risk_lanes']['V9.6.7_NO_MONTHLY_BUDGET']['trades']} |
| 部分窗口交易量复核条件 | {'达到' if blind_result['progress']['partial_trade_review_ready'] else '未达到'} |
| 正式胜者决策 | 禁止，部分月仅诊断 |
| 自动激活专家等级 | 否 |

只统计在截止时间前已经完成退出的交易；截止时仍未结束的信号按右删失处理，不计入胜负。账本保持追加式不可变。
...
"""
    ...
    (RESULTS / "run_identity.txt").write_text(
        ...
        f"risk_budget_winner_selection_enabled=False\nwinner_selection_enabled=False\nblind_start_month={BLIND['start_month']}\nblind_window_type={BLIND['window_type']}\nblind_window_start_utc={BLIND['window_start_utc']}\nblind_fixed_cutoff_utc={BLIND['fixed_cutoff_utc']}\nblind_complete_months={blind_result['progress']['complete_blind_month_count']}\nblind_partial_window_evaluated={blind_result['progress']['window_evaluated']}\nblind_decision_ready={blind_result['progress']['decision_ready']}\n",
        encoding="utf-8"
    )
    print(json.dumps({
        "tier_counts": counts,
        ...
        "blind_progress": blind_result["progress"],
...
if __name__=="__main__":
    ...
    if args.self_test:
        core.synthetic_smoke();statistical_self_test();attribution_self_test();shrinkage_allocation_self_test();original_gate_self_test();rating_source_self_test();blind_accumulation_self_test()
    elif args.pipeline_smoke:
        core.pipeline_smoke();statistical_self_test();attribution_self_test();shrinkage_allocation_self_test();original_gate_self_test();rating_source_self_test();blind_accumulation_self_test()
    else:main()
