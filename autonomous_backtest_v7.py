...
CACHE = ROOT / ".cache"
RESULTS = ROOT / "results_v7"
CACHE.mkdir(exist_ok=True)
RESULTS.mkdir(exist_ok=True)

ENGINE_VERSION = "V7"
ENGINE_NAME = "BTC 5m count-calibrated protected-exit ensemble V7"
...
def load_request() -> dict[str, Any]:
    ...
    path = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v7.json")))
    if path.exists():
        loaded = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise ValueError("request.v7.json must contain a JSON object")
        req.update(loaded)
    return req
...
if len(EVAL_MONTHS) != 2:
    raise ValueError("V7 requires exactly two evaluation months")
...
def download(url: str, path: Path, attempts: int = 6) -> bytes:
    ...
    for attempt in range(attempts):
        try:
            response = requests.get(url, timeout=90, headers={"User-Agent": "btc-count-calibrated-v7/1.0"})
...
def compute_outcomes(
    ...
    reasons = np.zeros(n, dtype=np.int8)  # 1 TP, 2 STOP, 3 TIME, 4 EARLY
    for k in range(n):
        signal_i = indices[k]
        entry_i = signal_i + 1
        ...
        for j in range(entry_i, end_i + 1):
            # Conservative intrabar ordering: the stop active at bar open is checked first.
            if direction > 0:
                if low[j] <= stop:
            ...
            # Stop adjustment applies only to the next bar, avoiding intrabar look-ahead.
        ...
        # V7 predicts whether the managed trade is profitable, including protected/time exits.
...
def fit_models(
    ...
    for side_data in (long_data, short_data):
        ...
        labels, _, _, _ = compute_outcomes(
            ...
            risk.rr, risk.sl_atr, risk.min_stop_pct, risk.max_hold, risk.breakeven_trigger, risk.breakeven_lock, risk.early_bars, risk.early_cut_r, FEE_RATE, SLIPPAGE_ABS,
        )
...
def events_from_bundle(
    bundle: dict[str, Any] | None,
    eval_month: str,
    train_months: set[str],
    monthly_candidate_target: float,
    regime_mask: int,
    setup_mask: int,
) -> list[dict[str, Any]]:
    if bundle is None:
        return []
    allowed = (
        np.isin(bundle["months"], list(train_months))
        & (bundle["exits"] >= 0)
        & np.array([((regime_mask >> int(r)) & 1) != 0 for r in bundle["regime"]], dtype=bool)
        & ((bundle["setup_mask"] & setup_mask) != 0)
    )
    train_prob = bundle["prob"][allowed]
    if len(train_prob) < 80:
        return []
    # Calibrate to a monthly raw-candidate budget using only historical training months.
    desired_total = max(1.0, monthly_candidate_target * max(1, len(train_months)))
    keep_fraction = min(0.45, max(0.004, desired_total / len(train_prob)))
    threshold = float(np.quantile(train_prob, 1.0 - keep_fraction))
    ...
    for k in np.flatnonzero(eval_allowed):
        percentile = float(np.searchsorted(sorted_train, bundle["prob"][k], side="right") / len(sorted_train))
        events.append({
...
def policy_events(
    long_bundle: dict[str, Any] | None,
    short_bundle: dict[str, Any] | None,
    policy: Policy,
    eval_month: str,
    train_months: set[str],
) -> list[dict[str, Any]]:
    events: list[dict[str, Any]] = []
    if policy.direction_mode in (0, 1):
        long_target = float(policy.candidate_target if policy.direction_mode == 1 else policy.candidate_target / 2.0)
        events += events_from_bundle(
            long_bundle, eval_month, train_months, long_target,
            policy.regime_mask, policy.setup_mask,
        )
    if policy.direction_mode in (0, 2):
        short_target = float(policy.candidate_target if policy.direction_mode == 2 else policy.candidate_target / 2.0)
        events += events_from_bundle(
            short_bundle, eval_month, train_months, short_target,
            policy.regime_mask, policy.setup_mask,
        )
    return events
...
def predict_events(
    ...
    bundle = build_prediction_bundle(x, side_data, model, risk, train_months)
    return events_from_bundle(
        bundle, eval_month, train_months, monthly_candidate_target,
        regime_mask, setup_mask,
    )
...
def select_trades(
    ...
    events = sorted(events, key=lambda e: (e["signal_i"], -e.get("score", e["prob"])))
    ...
    while i < len(events):
        ...
        while i < len(events) and events[i]["signal_i"] == signal_i:
            ...
            i += 1
        same.sort(key=lambda e: e.get("score", e["prob"]), reverse=True)
        if len(same) >= 2 and same[0]["direction"] != same[1]["direction"]:
            edge = same[0].get("score", same[0]["prob"]) - same[1].get("score", same[1]["prob"])
            if edge < edge_margin:
                continue
        best = same[0]
...
def month_score(m: dict[str, float], *, final_month: bool = False) -> float:
    count = int(m["trades"])
    target_min = MIN_TRADES if final_month else max(11, MIN_TRADES - 4)
    target_max = MAX_TRADES if final_month else MAX_TRADES + 5
    target = (MIN_TRADES + MAX_TRADES) / 2.0
    score = (
        m["win_rate"] * 1750.0
        + min(m["avg_win_loss_ratio"], 3.5) * 180.0
        + min(m["profit_factor"], 7.0) * 55.0
        + m["net_R"] * 8.0
        - m["max_drawdown_R"] * 7.0
        - abs(count - target) * 13.0
    )
    if count < target_min or count > target_max:
        score -= 4600.0 + abs(count - np.clip(count, target_min, target_max)) * 300.0
    if m["win_rate"] < MIN_WIN_RATE:
        score -= (MIN_WIN_RATE - m["win_rate"]) * 10500.0
    if m["avg_win_loss_ratio"] < MIN_RATIO:
        score -= (MIN_RATIO - m["avg_win_loss_ratio"]) * 4300.0
    if m["net_R"] <= 0:
        score -= 1100.0 + abs(m["net_R"]) * 45.0
    return float(score)
...
def evaluate_policy_batch(
    ...
    *,
    ...
    for (risk, model_cfg), group in groups.items():
        ...
        event_cache: dict[tuple[int, int, int, int], list[dict[str, Any]]] = {}
        for policy in group:
            key = (policy.candidate_target, policy.direction_mode, policy.regime_mask, policy.setup_mask)
            if key not in event_cache:
                event_cache[key] = policy_events(long_bundle, short_bundle, policy, eval_month, train_months)
            chosen = select_trades(
                event_cache[key], policy.cooldown, policy.direction_mode,
                policy.regime_mask, policy.setup_mask, policy.edge_margin,
            )
...
def evaluate_policy(
    ...
    long_bundle = build_prediction_bundle(x, long_data, long_model, policy.risk, train_months)
    short_bundle = build_prediction_bundle(x, short_data, short_model, policy.risk, train_months)
    events = policy_events(long_bundle, short_bundle, policy, eval_month, train_months)
    selected = select_trades(
        events, policy.cooldown, policy.direction_mode,
        policy.regime_mask, policy.setup_mask, policy.edge_margin,
    )
...
def main() -> None:
    ...
    # V7 lowers the nominal target, protects profitable excursions, cuts invalid trades early,
    # and calibrates signal thresholds to a historical monthly candidate budget.
    risk_grid: list[RiskConfig] = []
    for rr in (1.8, 2.0, 2.2):
        for sl in (1.1, 1.5):
            for min_stop in (0.0030, 0.0042):
                for hold in (144, 288):
                    if rr <= 2.0:
                        management = (0.95, 0.10, 48, 0.40)
                    else:
                        management = (1.15, 0.20, 72, 0.35)
                    risk_grid.append(RiskConfig(rr, sl, min_stop, hold, *management))
    model_grid = [
        ModelConfig(2, 0.040, 190, 2.0, 35),
        ModelConfig(3, 0.030, 230, 5.0, 50),
        ModelConfig(2, 0.025, 260, 8.0, 70),
    ]
    candidate_targets = (24, 32, 40, 52)
    cooldowns = (0, 3, 8)
    directions = (0, 1, 2)
    edge_margins = (0.00, 0.03, 0.06)
    regime_masks = (15, 1, 2, 4, 8, 5, 9)
    setup_masks = (15, 1, 2, 4, 8, 3, 5, 9, 12)

    # Sep-Nov train -> Dec broad prefilter.
    stage1: list[Policy] = []
            ...
            for target in candidate_targets:
                for cooldown in cooldowns:
                    for direction_mode in directions:
                        for edge_margin in edge_margins:
                            policy = Policy(risk, model_cfg, target, cooldown, direction_mode, edge_margin)
                            events = policy_events(long_bundle, short_bundle, policy, MONTHS[3], train0)
                            m = metrics(select_trades(
                                events, cooldown, direction_mode, 15, 15, edge_margin,
                            ))
    ...
    # Expand the strongest bases with state/setup filters and validate January.
    ...
    evaluate_policy_batch(x, long_data, short_data, expanded, set(MONTHS[:4]), MONTHS[4])
    expanded.sort(key=lambda p: p.aggregate_score, reverse=True)
    active = expanded[:72]

    # Rolling tournament through April. Worst-month performance has the largest weight.
    rolling = [
        (5, set(MONTHS[:5]), 58),
        (6, set(MONTHS[:6]), 46),
        (7, set(MONTHS[:7]), 34),
    ...
    # May is the final selection month; June is untouched until the policy is frozen.
    ...
    june_metrics, june_trades = evaluate_policy(x, long_data, short_data, selected, set(MONTHS[:9]), MONTHS[9])
    may_metrics, may_trades = evaluate_policy(x, long_data, short_data, selected, set(MONTHS[:8]), MONTHS[8])
    ...
    status = {
        ...
        "search_policies": len(stage1) + len(expanded),
    }
    ...
    for rank, policy in enumerate(active[:25], start=1):
        row: dict[str, Any] = {
            ...
            "breakeven_trigger": policy.risk.breakeven_trigger,
            "breakeven_lock": policy.risk.breakeven_lock,
            "early_bars": policy.risk.early_bars,
            "early_cut_r": policy.risk.early_cut_r,
            "candidate_target": policy.candidate_target,
            "cooldown": policy.cooldown,
            "direction": {0: "双向", 1: "只做多", 2: "只做空"}[policy.direction_mode],
            "edge_margin": policy.edge_margin,
    ...
    report = f"""# BTCUSDT 5分钟 计数校准与保护退出 V7 回测报告

- 数据：Binance USDⓈ-M 永续官方5分钟K线，{audit['actual_rows']:,}根；缺失0、重复0。
...
- 盈利保护：达到{selected.risk.breakeven_trigger:.2f}R后，下一根K线起保护{selected.risk.breakeven_lock:.2f}R
- 失败退出：持仓{selected.risk.early_bars}根后，最大顺向不足0.55R且亏损达到{selected.risk.early_cut_r:.2f}R时退出
- 历史月度候选目标：{selected.candidate_target}
- 多空冲突优势阈值：{selected.edge_margin:.3f}
