...
CACHE = ROOT / ".cache"
RESULTS = ROOT / "results_v9_4"
CACHE.mkdir(exist_ok=True)
RESULTS.mkdir(exist_ok=True)

ENGINE_VERSION = "V9.4"
ENGINE_NAME = "BTC 5m dual-core structure-locked expert ensemble V9.4"
OOS_MONTH = "2026-06"
...
DEFAULT_REQUEST: dict[str, Any] = {
    ...
    "expert_elimination": {
        ...
        "min_cumulative_profit_factor": 1.50,
        "min_cumulative_avg_win_loss_ratio": 1.50,
        "min_cumulative_net_r": 0.0,
        "max_worst_month_loss_r": 2.0,
        "max_single_month_profit_share": 0.45
    }
}
...
def load_request() -> dict[str, Any]:
    ...
    path = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_4.json")))
    if path.exists():
        loaded = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise ValueError("request.v9_4.json must contain a JSON object")
...
if len(EVAL_MONTHS) != 2:
    raise ValueError("V9.4 requires exactly two evaluation months")
...
if str(last_eval) != OOS_MONTH:
    raise ValueError(f"V9.4 hard-codes {OOS_MONTH} as the untouched out-of-sample month")
...
if CORE_EXPERT_IDS != (1, 2):
    raise ValueError("V9.4 fixed research design requires core experts [1, 2]")
if not (1 <= STAGE_MIN_TRADES <= STAGE_MAX_TRADES < MIN_TRADES):
    raise ValueError("V9.4 research stage must remain below the final minimum trade target")

if FEE_RATE != 0.0005:
    raise ValueError("V9.4 fixed requirement: fee_rate_per_side must be 0.0005")
if abs(SLIPPAGE_ABS - 0.2) > 1e-12:
    raise ValueError("V9.4 fixed requirement: each fill slippage must equal 0.2 USDT")
    ...
    for attempt in range(attempts):
        try:
            response = requests.get(
                url,
                timeout=90,
                headers={"User-Agent": "btc-dual-core-structure-locked-v9-4/1.0"},
            )
...
def add_features(
    ...
    # V9.4 signal funnel: 1h market state and 15m structure remain hard gates.
    ...
    # V9.4 adds a second, independent trend-short path: the first failed retest
...
def combine_events(
    ...
    V9.4 deliberately does not relax probability or percentile gates. Frequency
...
def quality_protection_test() -> None:
    ...
    if [e["signal_i"] for e in locked] != [10, 50]:
        raise RuntimeError("V9.4 structure-cycle loss lock failed")
    ...
    if len(setup_capped) != 1:
        raise RuntimeError("V9.4 per-setup daily cap failed")
    ...
    if len(expert_capped) != MAX_TRADES_PER_EXPERT_DAY:
        raise RuntimeError("V9.4 per-expert daily cap failed")
...
def self_test() -> None:
    ...
    if x.empty:
        raise RuntimeError("V9.4 self-test produced no feature rows")
    missing = [feature for feature in FEATURES if feature not in x.columns]
    if missing:
        raise RuntimeError(f"V9.4 self-test missing features: {missing}")
    ...
    if x_gap.empty:
        raise RuntimeError("V9.4 missing-auxiliary regression produced no rows")
    ...
    if set(experts) != set(range(6)):
        raise RuntimeError("V9.4 expert registry mismatch")
    ...
    if not funnel or not all(EXPERT_NAME[i] in funnel[next(iter(funnel))] for i in range(6)):
        raise RuntimeError("V9.4 signal funnel registry failed")
    if build_combinations({}) != []:
        raise RuntimeError("V9.4 empty combination must not be treated as a valid policy")
    ...
    if any(core_options[i] for i in range(6) if i not in CORE_EXPERT_IDS):
        raise RuntimeError("V9.4 diagnostic-only expert leaked into combination selection")
    ...
    if consecutive_zero_opportunity_months(probe) != 0:
        raise RuntimeError("V9.4 no-opportunity month was incorrectly treated as failure")
    if alignment_audit["eth_coverage_after_past_fill"] < 0.99:
        raise RuntimeError("Synthetic alignment unexpectedly failed")
    print(
        f"V9.4 self-test passed: rows={len(x)}, features={len(FEATURES)}, "
...
def main() -> None:
    ...
    selection_status = "CORE_RESEARCH_POLICY"
    ...
    research_stage_qualified = bool(stage_pass(may_metrics) and stage_pass(june_metrics))
    final_hard_metrics_passed = bool(final_metric_pass(may_metrics) and final_metric_pass(june_metrics))
    # May and June have already been inspected during prior iterations. Even if
    # metrics pass, V9.4 cannot be declared live-qualified without a fresh,
    # untouched complete blind month after the policy is frozen.
    qualified = False
    ...
    all_trades = pd.concat(nonempty, ignore_index=True) if nonempty else pd.DataFrame(columns=[
        ...
        "calibrated_probability", "online_score_percentile", "setup_group", "structure_cycle",
        "quality_protection", "entry", "risk_abs", "target_R", "net_R", "win", "exit_reason", "bars",
    ])
    ...
    elimination_status = {
        EXPERT_NAME[expert_id]: {
            ...
            "role": "core" if expert_id in CORE_EXPERT_IDS else "diagnostic_only",
        }
        for expert_id, candidates in all_candidates.items()
    }
    ...
    status = {
        ...
        "engine": ENGINE_NAME,
        "method": "Rolling holdout calibration; dual-core expert selection; structure-cycle loss lock; daily concentration controls; June evaluated once after freeze",
        "architecture": "completed 1h market state → completed 15m structure → independent 5m setup → holdout calibration → strict online rank → quality protection → next 5m open fill",
        "symbol": SYMBOL,
        "interval": INTERVAL,
        "months": list(MONTHS),
        "oos_month": OOS_MONTH,
        ...
        "constraints": {
            ...
            "core_expert_ids": list(CORE_EXPERT_IDS),
            "max_trades_per_expert_per_day": MAX_TRADES_PER_EXPERT_DAY,
            "max_trades_per_setup_per_day": MAX_TRADES_PER_SETUP_DAY,
            "max_consecutive_losses_per_expert_day": MAX_CONSECUTIVE_LOSSES_EXPERT_DAY,
            "loss_reentry_min_bars": LOSS_REENTRY_MIN_BARS,
        ...
        "diagnostic_only_experts": [EXPERT_NAME[i] for i in range(6) if i not in CORE_EXPERT_IDS],
        ...
        "quality_protection_audit": {
            MONTHS[8]: quality_protection_summary(may_trades),
            OOS_MONTH: quality_protection_summary(june_trades),
        },
        "signal_funnel_file": "results_v9_4/signal_funnel.csv",
        "may_stage_qualified_combinations": sum(stage_pass(combo.monthly_metrics[MONTHS[8]]) for combo in combinations),
    ...
    report = f"""# BTCUSDT 5分钟 双核心质量保护与结构锁定 V9.4 回测报告
...
- 质量保护：同一专家每天最多{MAX_TRADES_PER_EXPERT_DAY}笔，同一形态每天最多{MAX_TRADES_PER_SETUP_DAY}笔；止损后至少等待{LOSS_REENTRY_MIN_BARS}根5分钟K线且必须进入新的15分钟结构周期；连续{MAX_CONSECUTIVE_LOSSES_EXPERT_DAY}次亏损后当天停用该专家。
- 阶段目标：每月{STAGE_MIN_TRADES}–{STAGE_MAX_TRADES}笔、胜率≥{STAGE_MIN_WIN_RATE*100:.0f}%、实际盈亏比≥{STAGE_MIN_RATIO:.1f}、最大回撤≤{STAGE_MAX_DRAWDOWN:.1f}R。
- 原最终目标：每月{MIN_TRADES}–{MAX_TRADES}笔、胜率≥{MIN_WIN_RATE*100:.0f}%、实际盈亏比≥{MIN_RATIO:.1f}。
- 选择状态：**{selection_status}**（研究版，不可实盘）。
- 样本外：2026年6月未参与训练、阈值、专家淘汰或组合筛选，只在策略冻结后评估一次。
- V9.4阶段验收：**{'通过' if research_stage_qualified else '未通过'}**。
- 原最终指标：**{'数值通过，但仍需新盲测月' if final_hard_metrics_passed else '未通过'}**。
...
- 组合键：`{selected.key}`
- 5月达到V9.4阶段目标的候选组合：{status['may_stage_qualified_combinations']}
- 5月达到原最终硬指标的有效组合：{status['may_hard_qualified_combinations']}

## 月度结果

| 月份 | 交易 | 胜率 | 实际平均盈利/平均亏损 | 盈利因子 | 净R | 最大回撤R | 阶段通过 |
|---|---:|---:|---:|---:|---:|---:|---:|
| {MONTHS[8]} | {may_metrics['trades']} | {may_metrics['win_rate']*100:.2f}% | {may_metrics['avg_win_loss_ratio']:.3f} | {may_metrics['profit_factor']:.3f} | {may_metrics['net_R']:.3f} | {may_metrics['max_drawdown_R']:.3f} | {'是' if stage_pass(may_metrics) else '否'} |
| {OOS_MONTH} | {june_metrics['trades']} | {june_metrics['win_rate']*100:.2f}% | {june_metrics['avg_win_loss_ratio']:.3f} | {june_metrics['profit_factor']:.3f} | {june_metrics['net_R']:.3f} | {june_metrics['max_drawdown_R']:.3f} | {'是' if stage_pass(june_metrics) else '否'} |

## 质量保护审计

- 5月单日最多交易：{status['quality_protection_audit'][MONTHS[8]]['max_trades_same_day']}；同一专家单日最多：{status['quality_protection_audit'][MONTHS[8]]['max_trades_same_expert_day']}；同一形态单日最多：{status['quality_protection_audit'][MONTHS[8]]['max_trades_same_setup_day']}。
- 6月单日最多交易：{status['quality_protection_audit'][OOS_MONTH]['max_trades_same_day']}；同一专家单日最多：{status['quality_protection_audit'][OOS_MONTH]['max_trades_same_expert_day']}；同一形态单日最多：{status['quality_protection_audit'][OOS_MONTH]['max_trades_same_setup_day']}。
