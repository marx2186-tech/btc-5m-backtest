...
RESULTS = ROOT / "results_v9_6_6"
REQUEST_PATH = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_6_6.json")))
...
TRACK = REQUEST["triple_track_validation"]
SHRINK = REQUEST["evidence_shrinkage_lane"]
ENGINE_NAME = "BTC 5m uncertainty shrinkage and triple-track blind validation V9.6.6"

spec = importlib.util.spec_from_file_location("v966_strategy_core", ROOT / "_v966_strategy_core.py")
if spec is None or spec.loader is None:
    raise RuntimeError("Cannot load _v966_strategy_core.py")
...
def attribution_self_test() -> None:
    ...
    print("V966_TRIPLE_TRACK_SELF_TEST_OK")
...
def statistical_self_test()->None:
    ...
    print("V966_STATISTICAL_VALIDATION_SELF_TEST_OK")
...
def shrinkage_allocation_self_test() -> None:
    ...
    assert TRACK["winner_selection_enabled"] is False and TRACK["seen_oos_must_not_select_winner"] is True
    print("V966_EVIDENCE_SHRINKAGE_AND_ALLOCATION_OK")
...
def main()->None:
    ...
    snapshot={"engine":ENGINE_NAME,"engine_version":"V9.6.6","seed_expert_id":seed_id,"seed_expert":expert.name,"policy_key":seed_key,"policy":asdict(policy),"feature_group":expert.feature_group,"feature_list":list(core.FEATURE_GROUPS[expert.feature_group]),"training_months":list(core.DEVELOPMENT_MONTHS),"diagnostic_oos_month":core.OOS_MONTH,"execution":{"fee_rate_per_side":core.FEE_RATE,"slippage_abs":core.SLIPPAGE_ABS,"next_bar_open":True,"same_candle_stop_before_tp":True},"model_runtime":REQUEST["model"],"statistical_validation":STAT,"code_hashes":{"entrypoint":file_sha256(ROOT/"autonomous_backtest_v9_6_6.py"),"strategy_core":file_sha256(ROOT/"_v966_strategy_core.py"),"base_engine":file_sha256(ROOT/"_v966_base_engine.py"),"request":file_sha256(REQUEST_PATH)},"official_data_fingerprint":data_fp,"signal_fingerprint":original_fp,"random_seed":REQUEST["model"]["base_seed"]}
    ...
    # Full per-candidate stage attribution for three frozen lanes.
    ...
    trace_all = pd.concat([reference_trace, fixed_trace, shrink_trace], ignore_index=True, sort=False)
    ...
    if not fixed_trace_exact or not reference_trace_exact or not shrink_trace_exact:
        raise RuntimeError(f"Stage tracer mismatch: fixed={fixed_trace_exact} reference={reference_trace_exact} shrink={shrink_trace_exact}")
    ...
    def lane_view(df: pd.DataFrame, prefix: str) -> pd.DataFrame:
        ...
        keep = key_cols + [
            ...
            "utility", "utility_pass", "signal_class", "allocation_bucket", "cap_pass",
            "selected", "first_reject_stage"
        ]
    ...
    cmp["outcome_net_r"] = cmp.get("v961_net_r").combine_first(cmp.get("v964_net_r")).combine_first(cmp.get("v966_net_r"))
    cmp["outcome_win"] = cmp.get("v961_win").combine_first(cmp.get("v964_win")).combine_first(cmp.get("v966_win"))
    cmp["profit_rejected_v961"] = (~cmp["v961_selected"]) & (cmp["outcome_net_r"] > 0)
    cmp["profit_rejected_v964"] = (~cmp["v964_selected"]) & (cmp["outcome_net_r"] > 0)
    cmp["profit_rejected_v966"] = (~cmp["v966_selected"]) & (cmp["outcome_net_r"] > 0)
    cmp.to_csv(RESULTS / "triple_lane_candidate_diff.csv", index=False)
    cmp[[c for c in cmp.columns if not c.startswith("v966_")]].to_csv(RESULTS / "v961_vs_v964_candidate_diff.csv", index=False)
    ...
    utility_mask = shrink_trace["utility_pass"].fillna(False).astype(bool) if "utility_pass" in shrink_trace.columns else pd.Series(False, index=shrink_trace.index)
    cluster_mask = shrink_trace["first_reject_stage"].astype(str).eq("MARKET_CLUSTER_DUPLICATE") if "first_reject_stage" in shrink_trace.columns else pd.Series(False, index=shrink_trace.index)
    shrink_trace.loc[utility_mask, shrink_cols].to_csv(RESULTS / "quota_allocation_audit.csv", index=False)
    shrink_trace.loc[cluster_mask, shrink_cols].to_csv(RESULTS / "cluster_dedup_audit.csv", index=False)

    selected_all = pd.concat([reference_selected, fixed_selected, shrink_selected], ignore_index=True, sort=False)
    selected_all[selected_all["month"].astype(str).isin(dev_months)].to_csv(RESULTS / "triple_track_development_trades.csv", index=False)
    selected_all[selected_all["month"].astype(str) == core.OOS_MONTH].to_csv(RESULTS / "triple_track_oos_shadow.csv", index=False)

    block_rows_triple = block_quality_rows(reference_dev_selected, "V9.6.1_REFERENCE", dev_months)
    block_rows_triple += block_quality_rows(fixed_dev_selected, "V9.6.4_FIXED_PENALTY", dev_months)
    block_rows_triple += block_quality_rows(shrink_dev_selected, "V9.6.6_EVIDENCE_SHRINKAGE", dev_months)
    pd.DataFrame(block_rows_triple).to_csv(RESULTS / "block_quality_validation.csv", index=False)
    ...
    triple_payload = {
        ...
        "future_decision_rule": "Keep all three lanes frozen until a new blind window meets minimum calendar-month and trade-count evidence",
    ...
    (RESULTS / "triple_track_validation.json").write_text(json.dumps(triple_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    rule_delta = {
        ...
        "V9.6.6_EVIDENCE_SHRINKAGE": {"base_seed": int(core.MODEL["base_seed"]), "sample_uncertainty_mode": SHRINK["sample_uncertainty_mode"], "fixed_sample_penalty_r": float(SHRINK["fixed_sample_penalty_r"]), "allocation": SHRINK, "policy": asdict(policy)},
    ...
    status.update({"engine":ENGINE_NAME,"qualified":False,"not_for_live_trading":True,"tier_counts":counts,"statistical_validation_enabled":True,"monthly_raw_win_rate_is_not_validation":True,"selected_research_expert_count":counts["WATCH"]+counts["QUALIFIED"],"selected_qualified_expert_count":counts["QUALIFIED"],"selected_experts":{"research_watch_and_qualified_only":research_selected_payload,"qualified":qualified_selected_payload},"watch_portfolio_monthly_stats":research_stats,"research_portfolio_monthly_stats":research_stats,"qualified_portfolio_monthly_stats":qualified_stats,"snapshot_replay_exact":replay_audit["exact_match"],"v961_reference_exact_summary_match":exact_expected,"signal_stage_attribution_enabled":True,"triple_track_validation_enabled":True,"evidence_shrinkage_lane_enabled":True,"winner_selection_enabled":False,"winner_decision":"NO_WINNER_SELECTION_FROM_SEEN_OOS","fixed_stage_trace_exact":fixed_trace_exact,"reference_stage_trace_exact":reference_trace_exact,"shrinkage_stage_trace_exact":shrink_trace_exact,"blind_evidence_complete":False,"constraints":{**status.get("constraints",{}),"watch_gate":WATCH_GATE,"qualified_gate":QUALIFIED_GATE,"statistical_validation":STAT,"block_validation":BLOCK,"triple_track_validation":TRACK,"evidence_shrinkage_lane":SHRINK}})
    ...
    report=f"""# BTCUSDT 5分钟 信号筛选差异归因与三通道盲测 V9.6.6 报告
...
- 三条通道逐阶段追踪可复现：参考 `{reference_trace_exact}`；固定惩罚 `{fixed_trace_exact}`；证据收缩 `{shrink_trace_exact}`。
...
|---|---:|---:|---:|---:|---:|
| V9.6.1参考 | {triple_payload['lanes']['V9.6.1_REFERENCE']['development_metrics']['trades']} | {triple_payload['lanes']['V9.6.1_REFERENCE']['development_metrics']['win_rate']:.2%} | {triple_payload['lanes']['V9.6.1_REFERENCE']['development_metrics']['avg_win_loss_ratio']:.3f} | {triple_payload['lanes']['V9.6.1_REFERENCE']['development_metrics']['net_R']:.3f} | {triple_payload['lanes']['V9.6.1_REFERENCE']['development_metrics']['max_drawdown_R']:.3f} |
| V9.6.4固定惩罚 | {triple_payload['lanes']['V9.6.4_FIXED_PENALTY']['development_metrics']['trades']} | {triple_payload['lanes']['V9.6.4_FIXED_PENALTY']['development_metrics']['win_rate']:.2%} | {triple_payload['lanes']['V9.6.4_FIXED_PENALTY']['development_metrics']['avg_win_loss_ratio']:.3f} | {triple_payload['lanes']['V9.6.4_FIXED_PENALTY']['development_metrics']['net_R']:.3f} | {triple_payload['lanes']['V9.6.4_FIXED_PENALTY']['development_metrics']['max_drawdown_R']:.3f} |
| V9.6.6证据收缩 | {triple_payload['lanes']['V9.6.6_EVIDENCE_SHRINKAGE']['development_metrics']['trades']} | {triple_payload['lanes']['V9.6.6_EVIDENCE_SHRINKAGE']['development_metrics']['win_rate']:.2%} | {triple_payload['lanes']['V9.6.6_EVIDENCE_SHRINKAGE']['development_metrics']['avg_win_loss_ratio']:.3f} | {triple_payload['lanes']['V9.6.6_EVIDENCE_SHRINKAGE']['development_metrics']['net_R']:.3f} | {triple_payload['lanes']['V9.6.6_EVIDENCE_SHRINKAGE']['development_metrics']['max_drawdown_R']:.3f} |
...
if __name__=="__main__":
    ...
    if args.self_test:
        core.synthetic_smoke();statistical_self_test();attribution_self_test();shrinkage_allocation_self_test()
    elif args.pipeline_smoke:
        core.pipeline_smoke();statistical_self_test();attribution_self_test();shrinkage_allocation_self_test()
    else:main()
