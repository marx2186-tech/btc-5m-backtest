...
CACHE = ROOT / ".cache"
RESULTS = ROOT / "results_v9_5"
CACHE.mkdir(exist_ok=True)
RESULTS.mkdir(exist_ok=True)

ENGINE_VERSION = "V9.5"
ENGINE_NAME = "BTC 5m multi-model routed shadow ensemble V9.5"
OOS_MONTH = "2026-06"
...
def load_request() -> dict[str, Any]:
    ...
    path = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_5.json")))
    if path.exists():
        loaded = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(loaded, dict):
            raise ValueError("request.v9_5.json must contain a JSON object")
...
if len(EVAL_MONTHS) != 2:
    raise ValueError("V9.5 requires exactly two evaluation months")
...
if str(last_eval) != OOS_MONTH:
    raise ValueError(f"V9.5 hard-codes {OOS_MONTH} as the untouched out-of-sample month")
...
if CORE_EXPERT_IDS != (0, 1, 2, 3, 4, 5):
    raise ValueError("V9.5 requires six isolated directional specialists [0..5]")
if not (1 <= STAGE_MIN_TRADES <= STAGE_MAX_TRADES <= MIN_TRADES):
    raise ValueError("V9.5 research stage must not exceed the final minimum trade target")

if FEE_RATE != 0.0005:
    raise ValueError("V9.5 fixed requirement: fee_rate_per_side must be 0.0005")
if abs(SLIPPAGE_ABS - 0.2) > 1e-12:
    raise ValueError("V9.5 fixed requirement: each fill slippage must equal 0.2 USDT")
    ...
    for attempt in range(attempts):
        try:
            response = requests.get(
                url,
                timeout=90,
                headers={"User-Agent": "btc-multi-model-router-arbitrated-v9-5/1.0"},
            )
...
def add_features(
    ...
    # V9.5 signal funnel: 1h market state and 15m structure remain hard gates.
    ...
    # V9.5 adds a second, independent trend-short path: the first failed retest
...
def combine_events(
    ...
    V9.5 deliberately does not relax probability or percentile gates. Frequency
...
def quality_protection_test() -> None:
    ...
    if [e["signal_i"] for e in locked] != [10, 50]:
        raise RuntimeError("V9.5 structure-cycle loss lock failed")
    ...
    if len(setup_capped) != 1:
        raise RuntimeError("V9.5 per-setup daily cap failed")
    ...
    if len(expert_capped) != MAX_TRADES_PER_EXPERT_DAY:
        raise RuntimeError("V9.5 per-expert daily cap failed")
...
def self_test() -> None:
    ...
    if x.empty:
        raise RuntimeError("V9.5 self-test produced no feature rows")
    missing = [feature for feature in FEATURES if feature not in x.columns]
    if missing:
        raise RuntimeError(f"V9.5 self-test missing features: {missing}")
    ...
    if x_gap.empty:
        raise RuntimeError("V9.5 missing-auxiliary regression produced no rows")
    ...
    if set(experts) != set(range(6)):
        raise RuntimeError("V9.5 expert registry mismatch")
    ...
    if not funnel or not all(EXPERT_NAME[i] in funnel[next(iter(funnel))] for i in range(6)):
        raise RuntimeError("V9.5 signal funnel registry failed")
    if build_combinations({}) != []:
        raise RuntimeError("V9.5 empty combination must not be treated as a valid policy")
    ...
    if any(core_options[i] for i in range(6) if i not in CORE_EXPERT_IDS):
        raise RuntimeError("V9.5 diagnostic-only expert leaked into combination selection")
    ...
    if consecutive_zero_opportunity_months(probe) != 0:
        raise RuntimeError("V9.5 no-opportunity month was incorrectly treated as failure")
    if alignment_audit["eth_coverage_after_past_fill"] < 0.99:
        raise RuntimeError("Synthetic alignment unexpectedly failed")
    print(
        f"V9.5 self-test passed: rows={len(x)}, features={len(FEATURES)}, "
...
# ---------------------------------------------------------------------------
# V9.5 architecture overrides: isolated specialists, router, micro confirmation,
...
def evaluate_selected_month(
    ...
    if eval_month == OOS_MONTH and not selected.policies:
        raise RuntimeError("Cannot evaluate June without a frozen non-empty multi-model policy")
...
def prefer_complete_core_combinations(combinations: list[CombinationCandidate]) -> list[CombinationCandidate]:
    # V9.5 does not require all six models. It prioritizes true family diversity
...
def self_test() -> None:
    ...
    print("V95_SELF_TEST_OK")


def _fallback_multi_model_policy(all_candidates: dict[int, list[ExpertCandidate]]) -> CombinationCandidate:
    chosen: dict[int, ExpertPolicy] = {}
    # Prefer one usable model from each family, then fill remaining genuinely
    # active specialists. No model is invented when it has no development trades.
    for family in ("趋势延续", "区间流动性反转", "波动扩张突破"):
        family_candidates: list[ExpertCandidate] = []
        for expert_id, candidates in all_candidates.items():
            if MODEL_FAMILY[expert_id] != family:
                continue
            family_candidates.extend([
                c for c in candidates[:20]
                if sum(int(c.monthly_metrics.get(m, {}).get("trades", 0)) for m in DEVELOPMENT_MONTHS) > 0
            ])
        if family_candidates:
            best = max(family_candidates, key=lambda c: c.aggregate_score)
            chosen[best.policy.expert_id] = best.policy
    for expert_id, candidates in all_candidates.items():
        if expert_id in chosen:
            continue
        usable = [c for c in candidates[:10] if sum(int(c.monthly_metrics.get(m, {}).get("trades", 0)) for m in DEVELOPMENT_MONTHS) > 0]
        if usable:
            chosen[expert_id] = usable[0].policy
    if len(chosen) < 3:
        raise RuntimeError("NO_DIVERSE_MULTI_MODEL_FALLBACK")
    return CombinationCandidate(policies=chosen)
...
def main() -> None:
    ...
    if valid_combinations:
        ...
        if diagnostic_combinations:
            ...
            selected = _fallback_multi_model_policy(all_candidates)
    ...
    research_stage_qualified = bool(stage_pass(may_metrics, may_trades) and stage_pass(june_metrics, june_trades))
    ...
    status = {
        ...
        "selected_model_policies": selected_policy_payload(selected),
    ...
    report = f"""# BTCUSDT 5分钟 多模型隔离、影子回测与组合仲裁 V9.5 报告
...
- 选择状态：**{selection_status}**；无论数值是否达到，5月和6月已被查看，因此仍不可实盘。
- V9.5阶段验收：**{'通过' if research_stage_qualified else '未通过'}**。
- 原最终指标：**{'数值通过但需新盲测月' if final_hard_metrics_passed else '未通过'}**。

## 最终模型组合

- 活跃模型：{'、'.join(selected_names)}
- 覆盖模型家族：{'、'.join(sorted({MODEL_FAMILY[i] for i in selected.policies}))}
- 组合键：`{selected.key}`
...
if __name__ == "__main__":
    ...
    parser.add_argument("--self-test", action="store_true", help="run offline V9.5 architecture regression tests")
