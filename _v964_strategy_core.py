...
ENGINE_VERSION = "V9.6.4-core"
ENGINE_NAME = "BTC 5m reproducible sparse-expert core V9.6.4"
OOS_MONTH = "2026-06"
...
if EVAL_MONTHS != ("2026-05", "2026-06"):
    raise ValueError("V9.6.4 requires evaluation months 2026-05 and diagnostic OOS 2026-06")
...
if FEE_RATE != 0.0005 or abs(SLIPPAGE_ABS - 0.2) > 1e-12:
    raise ValueError("V9.6.4 fixes one-side fee at 0.050% and slippage at 0.2 USDT per fill")
...
compat = ROOT / ".v964_base_request.json"
...
spec = importlib.util.spec_from_file_location("v964_base_engine", ROOT / "_v964_base_engine.py")
...
def select_experts(best_by_expert:dict[int,Candidate])->tuple[list[Candidate],list[Candidate],str]:
    ...
    # V9.6.4-core invariant: CANDIDATE experts remain pure shadow and can never
...
def synthetic_smoke() -> None:
    ...
    print("V964_CORE_SELF_TEST_OK",json.dumps({"experts":len(EXPERTS),"nonzero_masks":sum(v>0 for v in counts.values()),"evidence_first":True,"mature_protected":True,"tier":tier},ensure_ascii=False))
...
def pipeline_smoke() -> None:
    ...
    smoke_dir=ROOT/".v964_pipeline_smoke"
    ...
    assert (smoke_dir/"candidate_summary.csv").exists();print("V964_CORE_PIPELINE_SMOKE_OK")
...
def main()->None:
    ...
    report=f"""# BTCUSDT 5分钟 证据优先参数选择与专家簇验证 V9.6.4-core 报告
...
V9.6.4-core的主要目标是防止低样本漂亮参数覆盖更有证据的成熟参数，并验证专家优势是否存在于一组邻近参数中。候选专家仍不进入组合。
"""
