...
RESULTS = ROOT / "results_v9_6_2"
RESULTS.mkdir(exist_ok=True)
ENGINE_VERSION = "V9.6.2"
ENGINE_NAME = "BTC 5m watch-first expert cultivation pool V9.6.2"
OOS_MONTH = "2026-06"

REQUEST_PATH = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_6_2.json")))
...
if EVAL_MONTHS != ("2026-05", "2026-06"):
    raise ValueError("V9.6.2 requires evaluation months 2026-05 and diagnostic OOS 2026-06")
...
if FEE_RATE != 0.0005 or abs(SLIPPAGE_ABS - 0.2) > 1e-12:
    raise ValueError("V9.6.2 fixes one-side fee at 0.050% and slippage at 0.2 USDT per fill")
...
compat = ROOT / ".v962_base_request.json"
...
spec = importlib.util.spec_from_file_location("v962_base_engine", ROOT / "_v962_base_engine.py")
if spec is None or spec.loader is None:
    raise RuntimeError("Cannot load _v962_base_engine.py")
...
def select_experts(best_by_expert:dict[int,Candidate])->tuple[list[Candidate],list[Candidate],str]:
    ...
    # V9.6.2 invariant: CANDIDATE experts remain pure shadow and can never
...
def synthetic_smoke() -> None:
    ...
    fake_watch=Candidate(policy_grid(EXPERTS[0])[0]);fake_watch.tier="WATCH";fake_watch.score=100
    fake_candidate=Candidate(policy_grid(EXPERTS[1])[0]);fake_candidate.tier="CANDIDATE";fake_candidate.score=999
    research,qualified,status=select_experts({0:fake_watch,1:fake_candidate})
    ...
    assert len(executable_events(fake_watch,[neutral]))==1
    assert not executable_events(fake_candidate,[support])
    assert len(candidate_support_events(fake_candidate,[support]))==1
    ...
    print("V962_SELF_TEST_OK",json.dumps({"experts":len(EXPERTS),"nonzero_masks":sum(v>0 for v in counts.values()),"candidate_shadow_only":True,"tier":tier},ensure_ascii=False))
...
def pipeline_smoke() -> None:
    ...
    smoke_dir=ROOT/".v962_pipeline_smoke"
    ...
    assert (smoke_dir/"candidate_summary.csv").exists();print("V962_PIPELINE_SMOKE_OK")
...
def main()->None:
    ...
    status_payload={"qualified":False,"research_stage_qualified":research_stage_ok,"final_hard_metrics_passed":final_ok,"not_for_live_trading":True,"fresh_blind_month_required":True,"selection_status":status,"engine":ENGINE_NAME,"architecture":"40 specific sparse experts -> candidate pure shadow -> WATCH/QUALIFIED research execution -> finite-sample utility penalty -> separate candidate SUPPORT experiment -> formal portfolio only after 3 qualified experts","tier_counts":tier_counts,"selected_research_expert_count":len(research_selected),"selected_qualified_expert_count":len(qualified_selected),"candidate_experts_excluded_from_research":True,"selected_experts":selected_json,"watch_portfolio_monthly_stats":research_metrics,"research_portfolio_monthly_stats":research_metrics,"qualified_portfolio_monthly_stats":qualified_metrics,"candidate_support_only_monthly_stats":support_metrics,"development_months":list(DEVELOPMENT_MONTHS),"constraints":{"candidate_gate":CANDIDATE_GATE,"watch_gate":WATCH_GATE,"qualified_gate":QUALIFIED_GATE,"research_stage":STAGE,"final_target":FINAL,"portfolio":PORT,"model":MODEL,"robustness":ROBUSTNESS},"oos_isolation":{"used_for_training":False,"used_for_thresholds":False,"used_for_expert_selection":False,"used_for_portfolio_selection":False,"evaluation_occurs_after_policy_freeze":True},"searched_experts":len(EXPERTS),"searched_policies":sum(len(v) for v in all_candidates.values())}
    ...
    report=f"""# BTCUSDT 5分钟 观察专家优先与候选纯影子 V9.6.2 报告
