...
RESULTS = ROOT / "results_v9_7_0"
REQUEST_PATH = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_7_0.json")))
...
ENGINE_NAME = "BTC 5m frozen baseline and append-only blind evidence accumulation V9.7.0"

spec = importlib.util.spec_from_file_location("v970_strategy_core", ROOT / "_v970_strategy_core.py")
if spec is None or spec.loader is None:
    raise RuntimeError("Cannot load _v970_strategy_core.py")
...
def build_unified_rating_shadow(
    ...
    for _, row in selected_policy_rows.sort_values("expert_id").iterrows():
        ...
        # V9.7.0 formalizes expert coverage as a request-level contract.
...
def attribution_self_test() -> None:
    ...
    print("V970_QUAD_TRACK_SELF_TEST_OK")
...
def statistical_self_test()->None:
    ...
    print("V970_STATISTICAL_VALIDATION_SELF_TEST_OK")
...
def shrinkage_allocation_self_test() -> None:
    ...
    assert TRACK["winner_selection_enabled"] is False and TRACK["seen_oos_must_not_select_winner"] is True
    print("V970_EVIDENCE_SHRINKAGE_AND_ALLOCATION_OK")
...
def original_gate_self_test() -> None:
    ...
    print("V970_ORIGINAL_SELECTION_AND_EXPERT_GATE_OK")
...
def rating_source_self_test() -> None:
    ...
    selftest_ref_request = ROOT / ".v970_selftest_reference_request.json"
    ...
    print("V970_UNIFIED_RATING_SOURCE_AND_DUAL_BUDGET_OK")
...
def _load_extended_frames(ref: Any, latest_month: str) -> tuple[pd.DataFrame,pd.DataFrame,dict[str,Any]]:
    all_months=_periods("2025-01",latest_month)
    old_core_months=core.base.MONTHS; old_ref_months=ref.base.MONTHS
    core.base.MONTHS=tuple(all_months); ref.base.MONTHS=tuple(all_months)
    try:
        raw,audit=core.base.load_official_data()
        eth,ea=core.base.load_auxiliary_kline("ETHUSDT","klines")
        premium,pa=core.base.load_auxiliary_kline(core.SYMBOL,"premiumIndexKlines")
        funding,fa=core.base.load_funding_rate()
        ...
        if not x.index.equals(rx.index):
            raise RuntimeError("V9.7.0 blind feature-frame index mismatch")
        return rx,x,{"official":audit,"eth":ea,"premium":pa,"funding":fa,"extended_alignment":align,"reference_alignment":ralign,"months":all_months}
    finally:
        core.base.MONTHS=old_core_months; ref.base.MONTHS=old_ref_months


def _discover_latest_archive(ref: Any, calendar_latest: str) -> tuple[str | None,pd.DataFrame | None,pd.DataFrame | None,dict[str,Any]]:
    start=pd.Period(str(BLIND["start_month"]),freq="M")
    candidate=pd.Period(calendar_latest,freq="M")
    attempts=[]
    while candidate>=start:
        try:
            rx,x,audit=_load_extended_frames(ref,str(candidate))
            return str(candidate),rx,x,{"attempts":attempts+[{"month":str(candidate),"available":True}],"data_audit":audit}
        except Exception as exc:
            attempts.append({"month":str(candidate),"available":False,"error":f"{type(exc).__name__}: {exc}"[:500]})
            candidate-=1
    return None,None,None,{"attempts":attempts,"data_audit":None}
...
def _blind_selected_for_experts(ref: Any, rx: pd.DataFrame, x: pd.DataFrame, tiers: pd.DataFrame, blind_months: list[str], expanded_months: list[str]) -> pd.DataFrame:
    ...
    for _,row in tiers.iterrows():
        ...
        selected["lane"]="V9.7.0_BLIND_NO_MONTHLY_BUDGET_ORIGINAL"; selected["rating_engine"]="V9.6.1_REFERENCE" if mod is ref else "V9.6.9_FROZEN_EXTENDED_ORIGINAL"
...
def _blind_outputs(previous_expert: pd.DataFrame, previous_risk: pd.DataFrame, ref: Any, tiers: pd.DataFrame, rating_shadow: pd.DataFrame, dev_months: list[str]) -> dict[str,Any]:
    calendar_latest=_calendar_latest_complete_month()
    start=str(BLIND["start_month"])
    calendar_months=_periods(start,calendar_latest)
    archive_latest,rx,x,discovery=_discover_latest_archive(ref,calendar_latest) if calendar_months else (None,None,None,{"attempts":[],"data_audit":None})
    available_months=_periods(start,archive_latest) if archive_latest else []
    current_month=str(pd.Period(pd.Timestamp.now(tz="UTC").strftime("%Y-%m"),freq="M"))
    registry=[]
    for m in _periods(start,current_month):
        p=pd.Period(m,freq="M")
        if m in available_months: state="AVAILABLE_COMPLETE_ARCHIVE"
        elif p<=pd.Period(calendar_latest,freq="M"): state="COMPLETE_ARCHIVE_PENDING"
        elif m==current_month: state="CURRENT_PARTIAL_NOT_EVALUATED"
        else: state="FUTURE"
        registry.append({"month":m,"state":state,"calendar_complete":bool(p<=pd.Period(calendar_latest,freq="M")),"archive_available":m in available_months,"used_for_tuning":False,"used_for_winner_selection":False})
    pd.DataFrame(registry).to_csv(RESULTS/"blind_month_registry.csv",index=False)
    new_expert=pd.DataFrame();new_risk=pd.DataFrame()
    if available_months and rx is not None and x is not None:
        expanded=_periods("2025-01",available_months[-1])
        new_expert=_blind_selected_for_experts(ref,rx,x,tiers,available_months,expanded)
        new_risk=_blind_seed_risk_lanes(ref,rx,tiers,available_months,expanded)
    ...
    # Provisional tiers: development evidence remains frozen; only completed blind trades are appended.
    provisional=[]
    evidence_months=dev_months+["2026-06"]+available_months
    for _,row in tiers.iterrows():
        eid=int(row["expert_id"]);key=str(row["policy_key"])
        dev=rating_shadow[(rating_shadow.get("expert_id",pd.Series(dtype=int)).astype(int)==eid)&(rating_shadow.get("policy_key",pd.Series(dtype=str)).astype(str)==key)&(rating_shadow.get("month",pd.Series(dtype=str)).astype(str).isin(dev_months))].copy() if not rating_shadow.empty else pd.DataFrame()
        blind=expert_ledger[(expert_ledger.get("expert_id",pd.Series(dtype=int)).astype(int)==eid)&(expert_ledger.get("policy_key",pd.Series(dtype=str)).astype(str)==key)].copy() if not expert_ledger.empty else pd.DataFrame()
        ...
        blind_trades=int(len(blind));months_complete=len(available_months)
        watch_ready=months_complete>=int(BLIND["minimum_complete_calendar_months"]) and blind_trades>=int(BLIND["minimum_blind_trades_watch_activation"])
        qual_ready=months_complete>=int(BLIND["minimum_complete_calendar_months"]) and blind_trades>=int(BLIND["minimum_blind_trades_qualified_activation"])
        activation_ready=(provisional_tier=="WATCH" and watch_ready) or (provisional_tier=="QUALIFIED" and qual_ready)
        provisional.append({"expert_id":eid,"expert":row["expert"],"policy_key":key,"frozen_tier":row["tier"],"provisional_tier":provisional_tier,"blind_trades":blind_trades,"complete_blind_months":months_complete,"watch_evidence_ready":watch_ready,"qualified_evidence_ready":qual_ready,"activation_ready_for_review":activation_ready,"automatic_activation":False,"rejection_reason":"|".join(reasons),**summary})
    provisional_df=pd.DataFrame(provisional);provisional_df.to_csv(RESULTS/"blind_provisional_tier_audit.csv",index=False)
    lane_progress={}
    for lane in ["V9.6.1_MONTHLY_BUDGET","V9.6.7_NO_MONTHLY_BUDGET"]:
        ...
        m=base_metrics(q);lane_progress[lane]={**m,"complete_calendar_months":len(available_months),"calendar_requirement_met":len(available_months)>=int(BLIND["minimum_complete_calendar_months"]),"trade_requirement_met":m["trades"]>=int(BLIND["minimum_trades_per_risk_lane"])}
        lane_progress[lane]["evidence_ready"]=bool(lane_progress[lane]["calendar_requirement_met"] and lane_progress[lane]["trade_requirement_met"])
    decision_ready=all(v["evidence_ready"] for v in lane_progress.values())
    progress={"blind_start_month":start,"calendar_latest_complete_month":calendar_latest,"latest_available_archive_month":archive_latest,"complete_blind_months":available_months,"complete_blind_month_count":len(available_months),"risk_lanes":lane_progress,"decision_ready":decision_ready,"winner_selection_enabled":False,"winner_decision":"REVIEW_REQUIRED_NO_AUTOMATIC_WINNER" if decision_ready else "INSUFFICIENT_FRESH_BLIND_EVIDENCE","automatic_tier_activation_enabled":False}
    ...
    (RESULTS/"blind_decision_readiness.json").write_text(json.dumps({"decision_ready":decision_ready,"requirements":{"minimum_complete_calendar_months":BLIND["minimum_complete_calendar_months"],"minimum_trades_per_risk_lane":BLIND["minimum_trades_per_risk_lane"]},"winner_selection_enabled":False,"action":"manual review in a later version" if decision_ready else "continue append-only accumulation"},ensure_ascii=False,indent=2),encoding="utf-8")
    integrity={"append_only_enabled":True,"expert_ledger":expert_merge,"risk_budget_ledger":risk_merge,"archive_discovery":discovery,"blind_data_used_for_policy_search":False,"blind_data_used_for_threshold_tuning":False,"blind_data_used_for_signal_formula_changes":False,"frozen_baseline_cutoff":REQUEST["release"]["frozen_baseline_cutoff"]}
...
def blind_accumulation_self_test() -> None:
    ...
    assert _calendar_latest_complete_month(pd.Timestamp("2026-07-27T12:00:00Z")) == "2026-06"
    assert _calendar_latest_complete_month(pd.Timestamp("2026-08-01T12:00:00Z")) == "2026-06"
    assert _calendar_latest_complete_month(pd.Timestamp("2026-08-06T12:00:00Z")) == "2026-07"
    ...
    print("V970_BLIND_APPEND_ONLY_SELF_TEST_OK")
...
def main()->None:
    ...
    if not x.index.equals(rx.index):
        raise RuntimeError(
            "V9.7.0 rating feature-frame index mismatch: "
    ...
    # V9.7.0 invariant: final expert evidence is rebuilt from the complete no-monthly-budget
    ...
    snapshot={"engine":ENGINE_NAME,"engine_version":"V9.7.0","seed_expert_id":seed_id,"seed_expert":expert.name,"policy_key":seed_key,"policy":asdict(policy),"feature_group":expert.feature_group,"feature_list":list(core.FEATURE_GROUPS[expert.feature_group]),"training_months":list(core.DEVELOPMENT_MONTHS),"diagnostic_oos_month":core.OOS_MONTH,"execution":{"fee_rate_per_side":core.FEE_RATE,"slippage_abs":core.SLIPPAGE_ABS,"next_bar_open":True,"same_candle_stop_before_tp":True},"model_runtime":REQUEST["model"],"statistical_validation":STAT,"code_hashes":{"entrypoint":file_sha256(ROOT/"autonomous_backtest_v9_7_0.py"),"strategy_core":file_sha256(ROOT/"_v970_strategy_core.py"),"base_engine":file_sha256(ROOT/"_v970_base_engine.py"),"request":file_sha256(REQUEST_PATH)},"official_data_fingerprint":data_fp,"signal_fingerprint":original_fp,"random_seed":REQUEST["model"]["base_seed"]}
    ...
    if bool(EXPERT_COVERAGE.get("require_complete_expert_coverage", True)) and not expert_id_coverage_complete:
        ...
        raise RuntimeError(f"V9.7.0 expert coverage incomplete: missing={missing_ids}, extra={extra_ids}")
    if bool(EXPERT_COVERAGE.get("require_complete_policy_coverage", True)) and not policy_coverage_complete:
        raise RuntimeError("V9.7.0 policy coverage incomplete; inspect rating_source_policy_coverage.csv")
    if not feature_frame_coverage_complete:
        ...
        raise RuntimeError(f"V9.7.0 feature-frame coverage incomplete: {bad}")
    rating_audit={
        "final_rating_source":str(RATING_SOURCE["source_lane"]),
    ...
    status.update({
        "engine": ENGINE_NAME,
        "release_version": "9.7.0",
        "release_type": "frozen_baseline_append_only_blind_evidence",
        "qualified": False,
    ...
    report = f"""# BTCUSDT 5分钟 冻结基线与追加式盲测证据 V9.7.0 报告
...
| 指标 | 结果 |
|---|---:|
| 盲测起始月 | {BLIND['start_month']} |
| 已有完整官方归档月 | {blind_result['progress']['complete_blind_month_count']} |
| 最新可用归档月 | {blind_result['progress']['latest_available_archive_month'] or '暂无'} |
...
| 达到人工决策条件 | {'是' if blind_result['progress']['decision_ready'] else '否'} |
| 自动选择胜者 | 否 |
| 自动激活专家等级 | 否 |
