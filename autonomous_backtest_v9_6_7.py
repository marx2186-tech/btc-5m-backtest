...
RESULTS = ROOT / "results_v9_6_7"
REQUEST_PATH = Path(os.environ.get("BACKTEST_REQUEST_FILE", str(ROOT / "request.v9_6_7.json")))
...
ORIGINAL = REQUEST["v967_original_lane"]
ENGINE_NAME = "BTC 5m expert-level evidence gating and frozen original selection V9.6.7"

spec = importlib.util.spec_from_file_location("v967_strategy_core", ROOT / "_v967_strategy_core.py")
if spec is None or spec.loader is None:
    raise RuntimeError("Cannot load _v967_strategy_core.py")
...
def trace_v967_original_lane(mod: Any, x: pd.DataFrame, expert: Any, policy: Any, months: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        trace, _ = trace_policy_lane(mod, x, expert, policy, "V9.6.7_EXPERT_GATE_ORIGINAL", months)
    finally:
...
def attribution_self_test() -> None:
    ...
    print("V967_QUAD_TRACK_SELF_TEST_OK")
...
def statistical_self_test()->None:
    ...
    print("V967_STATISTICAL_VALIDATION_SELF_TEST_OK")
...
def shrinkage_allocation_self_test() -> None:
    ...
    assert TRACK["winner_selection_enabled"] is False and TRACK["seen_oos_must_not_select_winner"] is True
    print("V967_EVIDENCE_SHRINKAGE_AND_ALLOCATION_OK")
...
def original_gate_self_test() -> None:
    ...
    print("V967_ORIGINAL_SELECTION_AND_EXPERT_GATE_OK")
...
def main()->None:
    ...
    snapshot={"engine":ENGINE_NAME,"engine_version":"V9.6.7","seed_expert_id":seed_id,"seed_expert":expert.name,"policy_key":seed_key,"policy":asdict(policy),"feature_group":expert.feature_group,"feature_list":list(core.FEATURE_GROUPS[expert.feature_group]),"training_months":list(core.DEVELOPMENT_MONTHS),"diagnostic_oos_month":core.OOS_MONTH,"execution":{"fee_rate_per_side":core.FEE_RATE,"slippage_abs":core.SLIPPAGE_ABS,"next_bar_open":True,"same_candle_stop_before_tp":True},"model_runtime":REQUEST["model"],"statistical_validation":STAT,"code_hashes":{"entrypoint":file_sha256(ROOT/"autonomous_backtest_v9_6_7.py"),"strategy_core":file_sha256(ROOT/"_v967_strategy_core.py"),"base_engine":file_sha256(ROOT/"_v967_base_engine.py"),"request":file_sha256(REQUEST_PATH)},"official_data_fingerprint":data_fp,"signal_fingerprint":original_fp,"random_seed":REQUEST["model"]["base_seed"]}
    ...
    v967_trace, v967_selected = trace_v967_original_lane(ref, rx, rexp, rpolicy, trace_months)
    _, v967_selected_replay = trace_v967_original_lane(ref, rx, rexp, rpolicy, trace_months)
    ...
    v967_gated.to_csv(RESULTS / "v967_original_shadow_trades.csv", index=False)
    v967_gated[v967_gated["risk_multiplier"] > 0].to_csv(RESULTS / "v967_research_execution_trades.csv", index=False)
    formal_ok = int((new_tiers["tier"] == "QUALIFIED").sum()) >= int(GATE["minimum_qualified_experts_for_formal"])
    if formal_ok:
        v967_gated[v967_gated["formal_eligible"]].to_csv(RESULTS / "v967_formal_execution_trades.csv", index=False)
    else:
        v967_gated.iloc[0:0].to_csv(RESULTS / "v967_formal_execution_trades.csv", index=False)
    ...
    if not fixed_trace_exact or not reference_trace_exact or not shrink_trace_exact or not v967_trace_exact:
        raise RuntimeError(f"Stage tracer mismatch: fixed={fixed_trace_exact} reference={reference_trace_exact} shrink={shrink_trace_exact} v967={v967_trace_exact}")
    ...
    block_rows_quad += block_quality_rows(v967_dev_selected, "V9.6.7_EXPERT_GATE_ORIGINAL", dev_months)
    ...
    (RESULTS / "triple_track_validation.json").write_text(json.dumps({**quad_payload, "lanes": {k:v for k,v in quad_payload["lanes"].items() if k != "V9.6.7_EXPERT_GATE_ORIGINAL"}}, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    rule_delta = {
        ...
        "V9.6.7_EXPERT_GATE_ORIGINAL": {"base_seed": int(ref.MODEL["base_seed"]), "signal_selection": ORIGINAL, "expert_level_gate": GATE, "policy": asdict(rpolicy)},
    ...
    status.update({
        ...
        "signal_selection_independent_of_tier": gate_independent,
        "v967_original_lane_trace_exact": v967_trace_exact,
        "v967_seed_tier": seed_tier,
        "v967_seed_risk_multiplier": float(GATE["tier_risk_multiplier"][seed_tier]),
        "v967_seed_execution_mode": str(GATE["tier_execution_mode"][seed_tier]),
        "v967_monthly_cap_enabled": bool(ORIGINAL["monthly_cap_enabled"]),
        "v967_cross_trade_cluster_dedup": bool(ORIGINAL["cross_trade_cluster_dedup"]),
        "winner_selection_enabled": False,
        "winner_decision": "NO_WINNER_SELECTION_FROM_SEEN_OOS",
    ...
    report = f"""# BTCUSDT 5分钟 专家级证据门控与原始选单冻结 V9.6.7 报告

- 架构：冻结V9.6.1原始信号质量 → 小样本统计只进入专家等级与风险门控 → 取消月度先到先得配额 → 仅保留持仓重叠、同根K线重复和每日风险限制。
...
- V9.6.7原始选单通道重放：**{'通过' if v967_trace_exact else '失败'}**。
- 专家等级是否改变信号集合：**{'否' if gate_independent else '是'}**。
- 月度交易配额：**关闭**；跨交易行情簇去重：**关闭**。
...
| V9.6.7专家门控原始选单 | {lanes['V9.6.7_EXPERT_GATE_ORIGINAL']['development_metrics']['trades']} | {lanes['V9.6.7_EXPERT_GATE_ORIGINAL']['development_metrics']['win_rate']:.2%} | {lanes['V9.6.7_EXPERT_GATE_ORIGINAL']['development_metrics']['avg_win_loss_ratio']:.3f} | {lanes['V9.6.7_EXPERT_GATE_ORIGINAL']['development_metrics']['net_R']:.3f} | {lanes['V9.6.7_EXPERT_GATE_ORIGINAL']['development_metrics']['max_drawdown_R']:.3f} |

`original_selection_attribution.csv`显示原始选单逐信号结果；`allocation_rule_audit.csv`确认没有月度配额和跨交易行情簇去重；`expert_level_gate_audit.csv`显示每个专家的风险倍数；`signal_selection_independence_audit.json`证明等级变化不改变信号集合。四条通道保持冻结，已查看OOS不选择胜者。
"""
    (RESULTS / "report.md").write_text(report, encoding="utf-8")
    (RESULTS / "run_identity.txt").write_text(
        f"{ENGINE_NAME}\noutput=results_v9_6_7\noos={core.OOS_MONTH}\n"
        ...
        f"shrinkage_stage_trace_exact={shrink_trace_exact}\nv967_stage_trace_exact={v967_trace_exact}\n"
        f"signal_selection_independent_of_tier={gate_independent}\nmonthly_cap_enabled={ORIGINAL['monthly_cap_enabled']}\n"
        f"cross_trade_cluster_dedup={ORIGINAL['cross_trade_cluster_dedup']}\nwinner_selection_enabled=False\n",
        encoding="utf-8"
    )
    print(json.dumps({
        "tier_counts": counts,
        "snapshot_replay_exact": replay_audit["exact_match"],
        "v961_reference_exact": exact_expected,
        "v967_trace_exact": v967_trace_exact,
        "signal_selection_independent_of_tier": gate_independent,
        "seed_tier": seed_tier,
        "seed_risk_multiplier": float(GATE["tier_risk_multiplier"][seed_tier]),
        "seed_evidence": seed_summary,
        "v967_development_metrics": lanes["V9.6.7_EXPERT_GATE_ORIGINAL"]["development_metrics"]
...
if __name__=="__main__":
    ...
    if args.self_test:
        core.synthetic_smoke();statistical_self_test();attribution_self_test();shrinkage_allocation_self_test();original_gate_self_test()
    elif args.pipeline_smoke:
        core.pipeline_smoke();statistical_self_test();attribution_self_test();shrinkage_allocation_self_test();original_gate_self_test()
    else:main()
