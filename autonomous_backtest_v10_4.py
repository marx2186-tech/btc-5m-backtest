...
import _v104_engine as engine

ROOT = Path(__file__).resolve().parent
RESULTS = ROOT / "results_v10_4"
DEFAULT_REQUEST = ROOT / "request.v10_4.json"
...
def validate_request(request: dict[str, Any]) -> None:
    ...
    if release["version"] != "10.4":
        raise ValueError("V10.4 requires release.version=10.4")
    if request["symbol"].upper() != "BTCUSDT":
        raise ValueError("V10.4 is frozen to BTCUSDT")
    if request["source_interval"] != "5m" or request["signal_timeframe_minutes"] != 60:
        raise ValueError("V10.4 must aggregate one-hour signals from official 5m data")
    if request["channel_count"] != 13:
        raise ValueError("V10.4 requires exactly 13 channels")
    if request["baseline_channel_count"] != 3:
        raise ValueError("V10.4 requires exactly 3 baselines")
    if request["directional_channel_count"] != 7 or request["portfolio_channel_count"] != 6:
        raise ValueError("V10.4 requires 7 directional and 6 portfolio channels")
    ...
    if not release["controlled_parameter_changes_only"]:
        raise ValueError("V10.4 permits only controlled parameter changes")
...
def pipeline_smoke(request: dict[str, Any]) -> None:
    synthetic = engine.synthetic_5m_data(110_000, seed=20261004)
    result = engine.run_benchmark(synthetic, request)
    if len(result["summary"]) != 13:
        raise AssertionError("Pipeline smoke did not return 13 channels")
    if len(result["baseline_comparison"]) != 13:
        raise AssertionError("Pipeline smoke did not create complete comparisons")
    if len(result["phases"]) != 39:
    ...
    if len(result["parameter_change_audit"]) != 5:
    ...
    print("V104_PIPELINE_SMOKE_OK")


def main() -> None:
    parser = argparse.ArgumentParser(description="BTCUSDT V10.4 direction-decoupled controlled optimization")
    ...
    status = {
        "engine": "BTCUSDT hourly direction-decoupled controlled optimization V10.4",
        "release_version": "10.4",
        ...
        "controlled_parameter_changes_only": True,
        "channel_count": 13,
        "directional_channel_count": 7,
        "portfolio_channel_count": 6,
        "parameter_change_count": 5,
        ...
        "next_step": "Select only single changes that improve their own frozen baseline; test combined long-short changes only in a later frozen interaction round.",
    }
    benchmark_summary = {
        ...
        "winner_decision": "HISTORICAL_CONTROLLED_SINGLE_CHANGE_DIAGNOSTIC_NO_AUTOMATIC_WINNER",
    }
    identity = {
        ...
        "engine_sha256": file_sha256(ROOT / "_v104_engine.py"),
        "runner_sha256": file_sha256(ROOT / "autonomous_backtest_v10_4.py"),
    }
    ...
    write_report(request, summary, phases, comparison, components, parameter_changes, leader)
    print(json.dumps(status, ensure_ascii=False, indent=2, default=str))
    print("V104_REAL_BACKTEST_COMPLETE")
